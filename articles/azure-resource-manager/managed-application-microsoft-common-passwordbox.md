...
Um controle que pode ser usado tooprovide e confirme uma senha. Use esse elemento ao [criar um Aplicativo Gerenciado do Azure](managed-application-publishing.md).
...
- Esse elemento não oferece suporte a saudação `defaultValue` propriedade.
...
- Se `options.hideConfirmation` está definido muito**true**, segunda caixa de texto Olá para confirmar a senha do usuário hello está oculto. valor padrão de saudação é **false**.
