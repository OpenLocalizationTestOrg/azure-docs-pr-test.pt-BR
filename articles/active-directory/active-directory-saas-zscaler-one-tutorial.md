...
Neste tutorial, você aprenderá como toointegrate Zscaler um com o Azure Active Directory (AD do Azure).

Integração do Zscaler um com o Azure AD oferece Olá benefícios a seguir:

- Você pode controlar no AD do Azure que tenha tooZscaler acesso um
- Você pode habilitar seu usuários tooautomatically get conectado tooZscaler um (logon único) com suas contas do AD do Azure
...
tooconfigure integração do AD do Azure com o Zscaler um, você precisa Olá itens a seguir:
...
1. Adicionando Zscaler um na Galeria de saudação
2. Configurar e testar o logon único do AD do Azure

## <a name="adding-zscaler-one-from-hello-gallery"></a>Adicionando Zscaler um na Galeria de saudação
integração de saudação tooconfigure do Zscaler um no AD do Azure, você precisa tooadd Zscaler um na lista de tooyour Olá Galeria de aplicativos SaaS gerenciados.

**tooadd Zscaler um na Galeria de hello, execute Olá etapas a seguir:**
...
4. Na caixa de pesquisa hello, digite **Zscaler um**.
...
5. No painel de resultados de saudação, selecione **Zscaler um**e, em seguida, clique em **adicionar** botão aplicativo hello de tooadd.
...
Para toowork de logon único, o AD do Azure precisa tooknow que usuário de contraparte Olá no Zscaler um é tooa usuário no AD do Azure. Em outras palavras, uma relação de link entre um usuário do AD do Azure e o usuário relacionado de saudação no Zscaler um precisa toobe estabelecida.

Zscaler um, atribuir valor Olá Olá **nome de usuário** no AD do Azure como valor de saudação do hello **Username** tooestablish relação de link de saudação.

tooconfigure e teste de logon único do AD do Azure com o Zscaler um, você precisa Olá toocomplete blocos de construção a seguir:
...
4. **[Criar um usuário de teste Zscaler um](#creating-a-zscaler-one-test-user)**  -toohave um equivalente do Britta Simon no Zscaler um que é vinculado toohello AD do Azure representação do usuário.
...
Nesta seção, habilitar o AD do Azure-logon único no hello portal do Azure e configurar o logon único no aplicativo Zscaler um.

**tooconfigure AD do Azure-logon único com o Zscaler um, execute Olá etapas a seguir:**

1. Em Olá portal do Azure, Olá **Zscaler um** página de integração de aplicativos, clique em **o logon único**.
...
3. Em Olá **Zscaler um domínio e URLs** , execute Olá etapas a seguir:

    ![Configurar Logon Único](./media/active-directory-saas-zscaler-one-tutorial/tutorial_zscalerone_url.png)

    Na caixa de texto de URL de entrada hello, digite Olá URL usado por seu usuários em toosign tooyour Zscaler um aplicativo.

    > [!NOTE] 
    > Você tem tooupdate esse valor com hello URL de logon real. Entre em contato com [equipe de suporte do Zscaler um cliente](https://www.zscaler.com/company/contact) tooget esses valores.
...
6. Em Olá **Zscaler uma configuração** seção, clique em **configurar Zscaler um** tooopen **configurar o logon** janela. Saudação de cópia **Single Sign-On URL do serviço SAML** de saudação **seção de referência rápida.**

    ![Configurar Logon Único](./media/active-directory-saas-zscaler-one-tutorial/tutorial_zscalerone_configure.png) 

7. Em uma janela de navegador web diferente, faça logon no Zscaler um site da empresa tooyour como um administrador.
...
toolog de usuários do AD do Azure de tooenable tooZscaler um, elas devem ser provisionado tooZscaler um. No caso de saudação do Zscaler um, o provisionamento é uma tarefa manual.
...
1. Faça logon no tooyour **Zscaler um** locatário.
...
> [!NOTE]
> Você pode usar outras Zscaler um usuário conta ferramentas de criação ou APIs fornecidas pelo Zscaler um tooprovision contas de usuário do AD do Azure.

### <a name="assigning-hello-azure-ad-test-user"></a>Atribuir um usuário de teste de saudação do AD do Azure

Nesta seção, você pode habilitar Britta Simon toouse logon único do Azure, concedendo tooZscaler acesso um.

![Atribuir usuário][200] 

**tooassign tooZscaler Britta Simon, execute Olá etapas a seguir:**

1. No hello portal do Azure, abra a exibição dos aplicativos Olá e navegue toohello exibição de diretório e ir muito**aplicativos empresariais** , em seguida, clique em **todos os aplicativos**.

    ![Atribuir usuário][201] 

2. Na lista de aplicativos hello, selecione **Zscaler um**.
...
Quando você clica em Zscaler um bloco Olá Olá painel de acesso, você deve obter automaticamente assinado em tooyour Zscaler um aplicativo.
