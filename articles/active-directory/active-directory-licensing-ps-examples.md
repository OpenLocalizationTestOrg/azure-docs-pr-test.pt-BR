...
A funcionalidade completa para licenciamento baseado em grupo está disponível por meio de saudação [portal do Azure](https://portal.azure.com), e atualmente o suporte do PowerShell é limitado. No entanto, há algumas tarefas úteis que podem ser executadas usando Olá existente [cmdlets do MSOnline PowerShell](https://docs.microsoft.com/powershell/msonline/v1/azureactivedirectory). Este documento fornece exemplos do que é possível.

> [!NOTE]
> Antes de começar a executar os cmdlets, certifique-se de conectar locatário tooyour pela primeira vez, executando Olá `Connect-MsolService` cmdlet.

## <a name="view-product-licenses-assigned-tooa-group"></a>Grupo de tooa atribuído de licenças de produto de exibição
O [Get-MsolGroup](/powershell/module/msonline/get-msolgroup?view=azureadps-1.0) cmdlet pode ser objeto de grupo usado tooretrieve hello e verificar Olá *licenças* propriedade: ele lista todas as licenças de produto atualmente atribuído toohello grupo.
```
...
> [!NOTE]
> dados saudação são informações limitadas tooproduct (SKU). Não é desabilitados na licença de saudação planos do serviço de Olá toolist possíveis.

## <a name="get-all-groups-with-licenses"></a>Obter todos os grupos com licenças

Você pode encontrar todos os grupos com qualquer licença atribuída executando Olá comando a seguir:
```
...
É possível relatar as estatísticas básicas para grupos com licenças. Exemplo hello seguir listamos contagem do total de usuários hello, Olá contagem de usuários com licenças já atribuídos pelo grupo hello e Olá contagem de usuários para quem licenças não podem ser atribuídas por grupo de saudação.
...
grupos de toofind que contêm alguns usuários para os quais não foi possível atribuir licenças:
```
...
Dado um grupo que contém alguns erros relacionados a licença, agora é possível listar todos os usuários afetados por esses erros. Um jser pode ter erros de outros grupos também. No entanto, neste exemplo é limitar resultados tooerrors toohello relevantes de grupo somente em questão, verificando o **ReferencedObjectId** propriedade de cada **IndirectLicenseError** a entrada do usuário de saudação.
...
```
## <a name="get-all-users-with-license-errors-in-hello-entire-tenant"></a>Obter todos os usuários com erros de licença no locatário todo Olá

toolist todos os usuários que possuem erros de licença de um ou mais grupos, Olá script a seguir podem ser usados. Esse script lista uma linha por usuário, por erro de licença que permite que você tooclearly identificar Olá origem de cada erro.

> [!NOTE]
> Esse script vai enumerar todos os usuários no locatário hello, que pode não ser ideal para grandes locatários.
...
Aqui está a outra versão do script hello que procura somente por meio de grupos que contêm erros de licença. Talvez seja que mais otimizado para cenários em que você espera toohave alguns grupos de problemas.
...
Para um objeto de usuário é possível toocheck se uma licença de produto em particular é atribuída de um grupo ou se ele for atribuído diretamente.

funções de exemplo Hello dois abaixo podem ser usado o tipo de saudação tooanalyze de atribuição de em um usuário individual:
```
...
Esse script executa as funções em cada usuário no locatário hello, usando a ID do SKU de saudação como entrada - neste exemplo estamos interessados na licença de saudação do *Enterprise Mobility + Security*, que, em nosso locatário é representada com ID *contoso:EMS*:
```
...
finalidade de saudação do script é tooremove as licenças de direto desnecessária de usuários que já herdam Olá mesmo de licença de um grupo; Por exemplo, como parte de um [fazendo a transição de licenciamento baseado em toogroup](https://docs.microsoft.com/azure/active-directory/active-directory-licensing-group-migration-azure-portal).
> [!NOTE]
> É importante toofirst validar esse Olá toobe direto licenças removida não habilitar a funcionalidade de serviço mais de saudação herdadas licenças. Caso contrário, a remover a licença direto Olá pode desabilitar tooservices de acesso e os dados para os usuários. Atualmente, não é possível toocheck por meio do PowerShell quais serviços estão habilitados por meio de licenças herdadas vs diretos. Script hello especificaremos o nível mínimo de saudação do services sabemos que estão sendo herdadas de grupos e verificaremos contra que.
...
toolearn mais sobre o recurso de saudação definida para o gerenciamento de licenças por meio de grupos, consulte o seguinte hello:
