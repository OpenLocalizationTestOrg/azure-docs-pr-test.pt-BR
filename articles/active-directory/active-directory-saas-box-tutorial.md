...
4. Na caixa de pesquisa, digite **Box**.

    ![Criação de um usuário de teste do AD do Azure](./media/active-directory-saas-box-tutorial/tutorial_box_search.png)

5. No painel de resultados, selecione **Box** e, em seguida, clique no botão **Adicionar** para adicionar o aplicativo.
...
Essa relação de vínculo é estabelecida por meio da atribuição do valor de **nome de usuário** no Azure AD como o valor de **Nome de Usuário** no Box.
...
3. **[Criação de um usuário de teste do Box](#creating-a-box-test-user)** – para ter um equivalente de Brenda Fernandes no Box que esteja vinculado à sua representação no Azure AD.
...
1. No portal do Azure, na página de integração de aplicativo do **Box**, clique em **Logon único**.
...
3. Na seção **URLs e Domínio do Box**, execute as seguintes etapas:

    ![Configurar Logon Único](./media/active-directory-saas-box-tutorial/tutorial_box_url.png)

    Na caixa de texto **URL de Logon**, digite uma URL usando o seguinte padrão: `https://<subdomain>.box.com`
...
Não há itens de ação para você nesta seção. Se um usuário ainda não existir no Box, um novo será criado quando você tentar acessar o Box.
...
**Para atribuir Brenda Fernandes ao Box, execute as seguintes etapas:**
...
Quando clicar no bloco do Box no Painel de Acesso, você deverá ser levado à página de logon para ser conectado automaticamente ao seu aplicativo Box.
