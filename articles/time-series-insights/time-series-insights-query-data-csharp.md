...
---
# <a name="query-data-from-hello-azure-time-series-insights-environment-using-c"></a><span data-ttu-id="f3b7e-103">Consultar dados do ambiente do Azure Insights de série de tempo hello usando c#</span><span class="sxs-lookup"><span data-stu-id="f3b7e-103">Query data from hello Azure Time Series Insights environment using C#</span></span>

<span data-ttu-id="f3b7e-104">Este exemplo c# demonstra como dados tooquery Olá ambiente do Azure Insights de série de tempo.</span><span class="sxs-lookup"><span data-stu-id="f3b7e-104">This C# example demonstrates how tooquery data from hello Azure Time Series Insights environment.</span></span>
<span data-ttu-id="f3b7e-105">exemplo Hello mostra vários exemplos básicos do uso da API de consulta:</span><span class="sxs-lookup"><span data-stu-id="f3b7e-105">hello sample shows several basic examples of Query API usage:</span></span>
1. <span data-ttu-id="f3b7e-106">Como uma etapa de preparação, adquira o token de acesso de saudação por meio de saudação API do Azure Active Directory.</span><span class="sxs-lookup"><span data-stu-id="f3b7e-106">As a preparation step, acquire hello access token through hello Azure Active Directory API.</span></span> <span data-ttu-id="f3b7e-107">Transmita esse token na Olá `Authorization` cabeçalho de cada solicitação de API de consulta.</span><span class="sxs-lookup"><span data-stu-id="f3b7e-107">Pass this token in hello `Authorization` header of every Query API request.</span></span> <span data-ttu-id="f3b7e-108">Para configurar aplicativos não interativos, veja [Autenticação e autorização](time-series-insights-authentication-and-authorization.md).</span><span class="sxs-lookup"><span data-stu-id="f3b7e-108">For setting up non-interactive applications, see [Authentication and authorization](time-series-insights-authentication-and-authorization.md).</span></span> <span data-ttu-id="f3b7e-109">Além disso, certifique-se de todas as constantes de saudação definidas no início de saudação do exemplo hello estão definidas corretamente.</span><span class="sxs-lookup"><span data-stu-id="f3b7e-109">Also, ensure all hello constants defined at hello beginning of hello sample are correctly set.</span></span>
2. <span data-ttu-id="f3b7e-110">lista de saudação dos ambientes que Olá usuário tem acesso toois obtido.</span><span class="sxs-lookup"><span data-stu-id="f3b7e-110">hello list of environments that hello user has access toois obtained.</span></span> <span data-ttu-id="f3b7e-111">Um dos ambientes de saudação é obtido como ambiente de saudação de interesse e ainda mais os dados são consultados para esse ambiente.</span><span class="sxs-lookup"><span data-stu-id="f3b7e-111">One of hello environments is picked up as hello environment of interest, and further data is queried for this environment.</span></span>
3. <span data-ttu-id="f3b7e-112">Como um exemplo de solicitação HTTPS, os dados de disponibilidade são solicitados para o ambiente de saudação de interesse.</span><span class="sxs-lookup"><span data-stu-id="f3b7e-112">As an example of HTTPS request, availability data is requested for hello environment of interest.</span></span>
4. <span data-ttu-id="f3b7e-113">Como um exemplo de solicitação do soquete da web, agrega dados de evento são solicitados para o ambiente de saudação de interesse.</span><span class="sxs-lookup"><span data-stu-id="f3b7e-113">As an example of web socket request, event aggregates data is requested for hello environment of interest.</span></span> <span data-ttu-id="f3b7e-114">Dados são necessários para o intervalo de tempo de disponibilidade de todo hello.</span><span class="sxs-lookup"><span data-stu-id="f3b7e-114">Data is requested for hello whole availability time range.</span></span>

## <a name="c-example"></a><span data-ttu-id="f3b7e-115">Exemplo de C#</span><span class="sxs-lookup"><span data-stu-id="f3b7e-115">C# example</span></span>
...
## <a name="next-steps"></a><span data-ttu-id="f3b7e-116">Próximas etapas</span><span class="sxs-lookup"><span data-stu-id="f3b7e-116">Next steps</span></span>

<span data-ttu-id="f3b7e-117">Para referência de API de consulta completa hello, consulte Olá [consulta API](/rest/api/time-series-insights/time-series-insights-reference-queryapi) documento.</span><span class="sxs-lookup"><span data-stu-id="f3b7e-117">For hello full Query API reference, see hello [Query API](/rest/api/time-series-insights/time-series-insights-reference-queryapi) document.</span></span>
