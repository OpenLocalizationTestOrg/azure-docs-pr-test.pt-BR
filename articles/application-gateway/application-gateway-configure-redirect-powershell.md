...
Gateway de aplicativo oferece suporte ao tráfego de tooredirect de capacidade de saudação com base em uma configuração definida. mais sobre o redirecionamento de visita geral, de toolearn [visão geral de redirecionamento do Application Gateway](application-gateway-redirect-overview.md). Este artigo fornece exemplos de tooHTTPS o redirecionamento de HTTP, o caminho com base redireciona redirecionamentos de vários locais e redireciona tooexternal sites.

## <a name="http-toohttps-redirect-on-an-existing-application-gateway"></a>Redirecionamento de HTTP tooHTTPS em um gateway de aplicativo existente

Olá exemplo a seguir cria um novo ouvinte HTTP na porta 80 tooredirect solicitações toohello existente escuta na porta 443.
...
Depois que o gateway de aplicativo hello for atualizado, teste o novo ouvinte Olá na porta 80.  Olá exemplo a seguir mostra o uso de `curl` redirecionamento Olá de teste.
...
saudação de exemplo a seguir cria um novo ouvinte e uma regra de caminho com base em um gateway de aplicativos existentes que redireciona somente o tráfego de caminho de url específico Olá de reunião.
...
Olá exemplo a seguir cria um novo gateway de aplicativo com 2 ouvintes de vários locais na porta 80. ouvintes de saudação são para adatum.com e adatum.org. Uma regra de redirecionamento é criada tooredirect tráfego de adatum.org tooadatum.com. Configuração adicional é necessária para configurar CNAME aliases toohello aplicativo gateway endereço IP público, para obter mais informações sobre como delegar tooAzure seu domínio DNS e registros para seu domínio criando CNAME visitarem [delegar a um domínio tooAzure DNS](../dns/dns-delegate-domain-azure-dns.md).
...
## <a name="redirect-tooan-external-site"></a>Redirecionar o site externo tooan

Olá, exemplo a seguir redireciona tráfego que chega ao site de externo tooan (bing.com) de gateway de aplicativo hello. Ao usar o hello `TargetUrl` comutador hello `IncludePath` e `IncludeQuery` opções não são permitidas.
...
Visite [configurar final tooend SSL com o application gateway usando o PowerShell](application-gateway-end-to-end-ssl-powershell.md)
