---
title: redirecionamento de aaaConfigure para Gateway de aplicativo do Azure - PowerShell | Microsoft Docs
description: "Esta página fornece redirecionamento de tooconfigure cenários para o Application Gateway utilizando o PowerShell"
...
---
# <a name="configure-redirection-on-application-gateway-with-powershell"></a><span data-ttu-id="4847e-103">Como configurar o redirecionamento no Gateway de Aplicativo com o PowerShell</span><span class="sxs-lookup"><span data-stu-id="4847e-103">Configure redirection on Application Gateway with PowerShell</span></span>

<span data-ttu-id="4847e-104">Gateway de aplicativo oferece suporte ao tráfego de tooredirect de capacidade de saudação com base em uma configuração definida.</span><span class="sxs-lookup"><span data-stu-id="4847e-104">Application gateway supports hello ability tooredirect traffic based on a defined configuration.</span></span> <span data-ttu-id="4847e-105">mais sobre o redirecionamento de visita geral, de toolearn [visão geral de redirecionamento do Application Gateway](application-gateway-redirect-overview.md).</span><span class="sxs-lookup"><span data-stu-id="4847e-105">toolearn more about redirection in general visit, [Application Gateway redirect overview](application-gateway-redirect-overview.md).</span></span> <span data-ttu-id="4847e-106">Este artigo fornece exemplos de tooHTTPS o redirecionamento de HTTP, o caminho com base redireciona redirecionamentos de vários locais e redireciona tooexternal sites.</span><span class="sxs-lookup"><span data-stu-id="4847e-106">This article provides examples of HTTP tooHTTPS redirection, path based redirects, multi-site redirects, and redirects tooexternal sites.</span></span>

## <a name="http-toohttps-redirect-on-an-existing-application-gateway"></a><span data-ttu-id="4847e-107">Redirecionamento de HTTP tooHTTPS em um gateway de aplicativo existente</span><span class="sxs-lookup"><span data-stu-id="4847e-107">HTTP tooHTTPS redirect on an existing application gateway</span></span>

<span data-ttu-id="4847e-108">Olá exemplo a seguir cria um novo ouvinte HTTP na porta 80 tooredirect solicitações toohello existente escuta na porta 443.</span><span class="sxs-lookup"><span data-stu-id="4847e-108">hello following example creates a new HTTP listener on port 80 tooredirect requests toohello existing listener on port 443.</span></span>
...
<span data-ttu-id="4847e-109">Depois que o gateway de aplicativo hello for atualizado, teste o novo ouvinte Olá na porta 80.</span><span class="sxs-lookup"><span data-stu-id="4847e-109">Once hello application gateway is updated, test hello new listener on port 80.</span></span>  <span data-ttu-id="4847e-110">Olá exemplo a seguir mostra o uso de `curl` redirecionamento Olá de teste.</span><span class="sxs-lookup"><span data-stu-id="4847e-110">hello following example shows using `curl` test hello redirection.</span></span>
...
## <a name="path-based-redirect"></a><span data-ttu-id="4847e-111">Redirecionamento baseado em caminho</span><span class="sxs-lookup"><span data-stu-id="4847e-111">Path based redirect</span></span>

<span data-ttu-id="4847e-112">saudação de exemplo a seguir cria um novo ouvinte e uma regra de caminho com base em um gateway de aplicativos existentes que redireciona somente o tráfego de caminho de url específico Olá de reunião.</span><span class="sxs-lookup"><span data-stu-id="4847e-112">hello following example creates a new listener and a path based rule on an existing application gateway that redirects only traffic meeting hello specific url path.</span></span>
...
## <a name="multi-site-redirect"></a><span data-ttu-id="4847e-113">Redirecionamento de vários locais</span><span class="sxs-lookup"><span data-stu-id="4847e-113">Multi-site redirect</span></span>

<span data-ttu-id="4847e-114">Olá exemplo a seguir cria um novo gateway de aplicativo com 2 ouvintes de vários locais na porta 80.</span><span class="sxs-lookup"><span data-stu-id="4847e-114">hello following example creates a new application gateway with 2 multi-site listeners on port 80.</span></span> <span data-ttu-id="4847e-115">ouvintes de saudação são para adatum.com e adatum.org. Uma regra de redirecionamento é criada tooredirect tráfego de adatum.org tooadatum.com. Configuração adicional é necessária para configurar CNAME aliases toohello aplicativo gateway endereço IP público, para obter mais informações sobre como delegar tooAzure seu domínio DNS e registros para seu domínio criando CNAME visitarem [delegar a um domínio tooAzure DNS](../dns/dns-delegate-domain-azure-dns.md).</span><span class="sxs-lookup"><span data-stu-id="4847e-115">hello listeners are for adatum.com and adatum.org. A redirect rule is created tooredirect traffic from adatum.org tooadatum.com. Additional configuration is required for configuring CNAME aliases toohello application gateway public IP address, for more information on delegating your domain tooAzure DNS and creating CNAME records for your domain visit, [Delegate a domain tooAzure DNS](../dns/dns-delegate-domain-azure-dns.md).</span></span>
...
## <a name="redirect-tooan-external-site"></a><span data-ttu-id="4847e-116">Redirecionar o site externo tooan</span><span class="sxs-lookup"><span data-stu-id="4847e-116">Redirect tooan external site</span></span>

<span data-ttu-id="4847e-117">Olá, exemplo a seguir redireciona tráfego que chega ao site de externo tooan (bing.com) de gateway de aplicativo hello.</span><span class="sxs-lookup"><span data-stu-id="4847e-117">hello following example redirects traffic coming into hello application gateway tooan external website (bing.com).</span></span> <span data-ttu-id="4847e-118">Ao usar o hello `TargetUrl` comutador hello `IncludePath` e `IncludeQuery` opções não são permitidas.</span><span class="sxs-lookup"><span data-stu-id="4847e-118">When using hello `TargetUrl` switch hello `IncludePath` and `IncludeQuery` switches are not allowed.</span></span>
...
## <a name="next-steps"></a><span data-ttu-id="4847e-119">Próximas etapas</span><span class="sxs-lookup"><span data-stu-id="4847e-119">Next steps</span></span>

<span data-ttu-id="4847e-120">Visite [configurar final tooend SSL com o application gateway usando o PowerShell](application-gateway-end-to-end-ssl-powershell.md)</span><span class="sxs-lookup"><span data-stu-id="4847e-120">Visit [configure end tooend SSL with application gateway using PowerShell](application-gateway-end-to-end-ssl-powershell.md)</span></span>
