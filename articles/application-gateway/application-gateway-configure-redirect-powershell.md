...
---
# <a name="configure-redirection-on-application-gateway-with-powershell"></a><span data-ttu-id="9aee5-103">Como configurar o redirecionamento no Gateway de Aplicativo com o PowerShell</span><span class="sxs-lookup"><span data-stu-id="9aee5-103">Configure redirection on Application Gateway with PowerShell</span></span>

<span data-ttu-id="9aee5-104">O gateway de aplicativo suporta a capacidade de redirecionar o tráfego com base em uma configuração definida.</span><span class="sxs-lookup"><span data-stu-id="9aee5-104">Application gateway supports the ability to redirect traffic based on a defined configuration.</span></span> <span data-ttu-id="9aee5-105">Para saber mais sobre o redirecionamento de uma maneira geral, confira: [Visão geral de redirecionamento do Gateway de Aplicativo](application-gateway-redirect-overview.md).</span><span class="sxs-lookup"><span data-stu-id="9aee5-105">To learn more about redirection in general visit, [Application Gateway redirect overview](application-gateway-redirect-overview.md).</span></span> <span data-ttu-id="9aee5-106">Este artigo fornece exemplos de redirecionamento de HTTP para HTTPS, redirecionamentos baseados em caminho, redirecionamentos de vários locais e redirecionamentos para sites externos.</span><span class="sxs-lookup"><span data-stu-id="9aee5-106">This article provides examples of HTTP to HTTPS redirection, path based redirects, multi-site redirects, and redirects to external sites.</span></span>

## <a name="http-to-https-redirect-on-an-existing-application-gateway"></a><span data-ttu-id="9aee5-107">Redirecionamento de HTTP para HTTPS em um gateway de aplicativo existente</span><span class="sxs-lookup"><span data-stu-id="9aee5-107">HTTP to HTTPS redirect on an existing application gateway</span></span>

<span data-ttu-id="9aee5-108">O exemplo a seguir cria um novo ouvinte HTTP na porta 80 para redirecionar solicitações para o ouvinte existente na porta 443.</span><span class="sxs-lookup"><span data-stu-id="9aee5-108">The following example creates a new HTTP listener on port 80 to redirect requests to the existing listener on port 443.</span></span>
...
<span data-ttu-id="9aee5-109">Depois que o gateway de aplicativo for atualizado, teste o novo ouvinte na porta 80.</span><span class="sxs-lookup"><span data-stu-id="9aee5-109">Once the application gateway is updated, test the new listener on port 80.</span></span>  <span data-ttu-id="9aee5-110">O exemplo a seguir mostra o uso de teste de redirecionamento `curl`.</span><span class="sxs-lookup"><span data-stu-id="9aee5-110">The following example shows using `curl` test the redirection.</span></span>
...
## <a name="path-based-redirect"></a><span data-ttu-id="9aee5-111">Redirecionamento baseado em caminho</span><span class="sxs-lookup"><span data-stu-id="9aee5-111">Path based redirect</span></span>

<span data-ttu-id="9aee5-112">O exemplo a seguir cria um novo ouvinte e uma regra baseada em caminho em um gateway de aplicativo existente que redireciona somente o tráfego que atende o caminho da url específica.</span><span class="sxs-lookup"><span data-stu-id="9aee5-112">The following example creates a new listener and a path based rule on an existing application gateway that redirects only traffic meeting the specific url path.</span></span>
...
## <a name="multi-site-redirect"></a><span data-ttu-id="9aee5-113">Redirecionamento de vários locais</span><span class="sxs-lookup"><span data-stu-id="9aee5-113">Multi-site redirect</span></span>

<span data-ttu-id="9aee5-114">O exemplo a seguir cria um novo gateway de aplicativo com 2 ouvintes de vários locais na porta 80.</span><span class="sxs-lookup"><span data-stu-id="9aee5-114">The following example creates a new application gateway with 2 multi-site listeners on port 80.</span></span> <span data-ttu-id="9aee5-115">Ouvintes para adatum.com e adatum.org.</span><span class="sxs-lookup"><span data-stu-id="9aee5-115">The listeners are for adatum.com and adatum.org.</span></span> <span data-ttu-id="9aee5-116">Cria-se uma regra de redirecionamento para redirecionar o tráfego de adatum.org para adatum.com.</span><span class="sxs-lookup"><span data-stu-id="9aee5-116">A redirect rule is created to redirect traffic from adatum.org to adatum.com.</span></span> <span data-ttu-id="9aee5-117">Uma configuração adicional é necessária para configurar o antigo CNAME para o endereço IP público do gateway de aplicativo, para obter mais informações sobre delegação de seu domínio DNS do Azure e criação de registros de CNAME para o seu domínio, visite [Como delegar um domínio DNS do Azure](../dns/dns-delegate-domain-azure-dns.md).</span><span class="sxs-lookup"><span data-stu-id="9aee5-117">Additional configuration is required for configuring CNAME aliases to the application gateway public IP address, for more information on delegating your domain to Azure DNS and creating CNAME records for your domain visit, [Delegate a domain to Azure DNS](../dns/dns-delegate-domain-azure-dns.md).</span></span>
...
## <a name="redirect-to-an-external-site"></a><span data-ttu-id="9aee5-118">Redirecionamento para um site externo</span><span class="sxs-lookup"><span data-stu-id="9aee5-118">Redirect to an external site</span></span>

<span data-ttu-id="9aee5-119">O exemplo a seguir redireciona o tráfego que entra do gateway de aplicativo para um site externo (bing.com).</span><span class="sxs-lookup"><span data-stu-id="9aee5-119">The following example redirects traffic coming into the application gateway to an external website (bing.com).</span></span> <span data-ttu-id="9aee5-120">Ao usar o comutador `TargetUrl`, os comutadores `IncludePath` e `IncludeQuery` não são permitidos.</span><span class="sxs-lookup"><span data-stu-id="9aee5-120">When using the `TargetUrl` switch the `IncludePath` and `IncludeQuery` switches are not allowed.</span></span>
...
## <a name="next-steps"></a><span data-ttu-id="9aee5-121">Próximas etapas</span><span class="sxs-lookup"><span data-stu-id="9aee5-121">Next steps</span></span>

<span data-ttu-id="9aee5-122">Confira: [Como configurar um SSL de ponta a ponta com um gateway de aplicativo usando o PowerShell](application-gateway-end-to-end-ssl-powershell.md)</span><span class="sxs-lookup"><span data-stu-id="9aee5-122">Visit [configure end to end SSL with application gateway using PowerShell](application-gateway-end-to-end-ssl-powershell.md)</span></span>