...
Este documento é uma referência para a API. Você deve começar pelo documento "Recomendação do Azure Machine Learning - Início Rápido".

A API de Recomendações do Azure Machine Learning pode ser dividida nos seguintes grupos lógicos:
...
O URI da raiz de serviço para as APIs de Recomendações do Azure Machine Learning está [aqui.](https://api.datamarket.azure.com/amla/recommendations/v3/)
...
* <strong>WhiteList</strong> – WhiteList permite a você sugerir recomendações apenas de uma lista de itens.
...
* Configure uma compilação de recomendação com os seguintes parâmetros:
  ...
  * `AllowColdItemPlacement` – definido como True.
  * Opcionalmente, você pode definir `EnableFeatureCorrelation` como True e `ReasoningFeatureList` para a lista de recursos que deseja usar para obter explicações (normalmente é a mesma lista de recursos usada na modelagem ou uma sublista).
...
As Recomendações do Azure Machine Learning criam notificações quando erros persistentes ocorrem no sistema. Há três tipos de notificações:
