...
tooaccess armazenamento do Azure, é necessário toopass nas credenciais de armazenamento. toodo isso, utilize as configurações do aplicativo Web. config.
Essas configurações serão passadas como tooNode de variáveis de ambiente, que são então lidos pelo Olá SDK do Azure.

> [!NOTE]
> As credenciais de armazenamento são usadas somente quando o aplicativo hello é tooAzure implantado. Quando em execução no emulador hello, aplicativo hello usará o emulador de armazenamento hello.
>
>
...
3. Na janela do Powershell do Azure Olá digite Olá informações de conta de armazenamento do cmdlet tooretrieve Olá a seguir:

    ```powershell
    PS C:\node\tasklist\WebRole1> Get-AzureStorageAccounts
    ```

   Isso recupera a lista de saudação de contas de armazenamento e chaves associados ao seu serviço hospedado de conta.
...
Nesta seção, você estenderá aplicativo básico do hello criado pelo Olá **express** comando adicionando um **task.js** arquivo que contém o modelo de saudação para suas tarefas. Você também modificará Olá existente **app.js** e criar um novo **tasklist.js** arquivo que usa o modelo de saudação.
...
4. Em seguida, você adicionará código toodefine e exportar o objeto de tarefa hello. Esse objeto é responsável por conectar-se a tabela de toohello.
...
2. Substitua o conteúdo de saudação do hello **Jade** arquivo com o código de saudação abaixo. Isso define o modo de exibição de saudação para exibir as tarefas existentes, bem como um formulário para adicionar novas tarefas e marcar os existentes, como concluída.
...
Olá **layout.jade** arquivo hello **exibições** diretório é usado como um modelo global para outros **.jade** arquivos. Nesta etapa você modificá-la toouse [Twitter Bootstrap](https://github.com/twbs/bootstrap), que é um kit de ferramentas que torna mais fácil toodesign um site de aparência adequado.
...
2. De saudação **exibições** pasta, abra Olá **layout.jade** em seu editor e substituir Olá conteúdo de texto com os seguintes hello:
...
navegador de saudação será aberto e exibe o saudação página a seguir:
...
Como antes, porque você especificou Olá **-iniciar** opção, o navegador Olá abre e exibe seu aplicativo em execução no Azure, quando a publicação é concluída.

![Uma janela do navegador exibindo a página do hello minha lista de tarefas. URL de saudação indica a página Olá agora está sendo hospedada no Azure.](./media/storage-nodejs-use-table-storage-cloud-service-app/getting-started-1.png)
