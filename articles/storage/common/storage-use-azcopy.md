...
## <a name="download-and-install-azcopy"></a><span data-ttu-id="5798a-112">Baixar e instalar o AzCopy</span><span class="sxs-lookup"><span data-stu-id="5798a-112">Download and install AzCopy</span></span>
...
### <a name="download-blobs-with-specified-prefix"></a><span data-ttu-id="5798a-132">Baixar blobs com prefixo especificado</span><span class="sxs-lookup"><span data-stu-id="5798a-132">Download blobs with specified prefix</span></span>
...
### <a name="upload-all-files"></a><span data-ttu-id="5798a-148">Carregar todos os arquivos</span><span class="sxs-lookup"><span data-stu-id="5798a-148">Upload all files</span></span>
...
<span data-ttu-id="5798a-155">Suponha seguinte Olá arquivos residem na pasta `C:\myfolder`:</span><span class="sxs-lookup"><span data-stu-id="5798a-155">Assume hello following files reside in folder `C:\myfolder`:</span></span>
...
<span data-ttu-id="5798a-169">Observe que você deve ter um armazenamento com redundância geográfica e acesso de leitura habilitado.</span><span class="sxs-lookup"><span data-stu-id="5798a-169">Note that you must have read-access geo-redundant storage enabled.</span></span>
...
## <a name="table-export"></a><span data-ttu-id="5798a-202">Tabela: exportar</span><span class="sxs-lookup"><span data-stu-id="5798a-202">Table: Export</span></span>
...
## <a name="table-import"></a><span data-ttu-id="5798a-235">Tabela: importar</span><span class="sxs-lookup"><span data-stu-id="5798a-235">Table: Import</span></span>
...
### <a name="import-entities-tootable-using-blobs"></a><span data-ttu-id="5798a-250">Importar entidades tootable usando blobs</span><span class="sxs-lookup"><span data-stu-id="5798a-250">Import entities tootable using blobs</span></span>
...
### <a name="use-a-response-file-toospecify-command-line-parameters"></a><span data-ttu-id="5798a-258">Usar parâmetros de linha de comando do arquivo toospecify uma resposta</span><span class="sxs-lookup"><span data-stu-id="5798a-258">Use a response file toospecify command-line parameters</span></span>
...
### <a name="generate-a-log-file"></a><span data-ttu-id="5798a-287">Gerar um arquivo de log</span><span class="sxs-lookup"><span data-stu-id="5798a-287">Generate a log file</span></span>
...
### <a name="run-azcopy-against-azure-storage-emulator"></a><span data-ttu-id="5798a-297">Executar o AzCopy em um emulador de armazenamento do Azure</span><span class="sxs-lookup"><span data-stu-id="5798a-297">Run AzCopy against Azure Storage Emulator</span></span>
...
<span data-ttu-id="5798a-309">**Aplicável a:** Blobs, Arquivos, Tabelas</span><span class="sxs-lookup"><span data-stu-id="5798a-309">**Applicable to:** Blobs, Files, Tables</span></span>
...
<span data-ttu-id="5798a-351">**Aplicável a:** Blobs, Arquivos</span><span class="sxs-lookup"><span data-stu-id="5798a-351">**Applicable to:** Blobs, Files</span></span>
...
<span data-ttu-id="5798a-368">Olá instantâneos de blob transferidos são renomeados neste formato: nome de blob (hora do instantâneo) .extension</span><span class="sxs-lookup"><span data-stu-id="5798a-368">hello transferred blob snapshots are renamed in this format: blob-name (snapshot-time).extension</span></span>
...
<span data-ttu-id="5798a-389">Especifica um arquivo que contém parâmetros.</span><span class="sxs-lookup"><span data-stu-id="5798a-389">Specifies a file that contains parameters.</span></span> <span data-ttu-id="5798a-390">AzCopy processos Olá parâmetros no arquivo hello como se eles tivessem sido especificados na linha de comando hello.</span><span class="sxs-lookup"><span data-stu-id="5798a-390">AzCopy processes hello parameters in hello file just as if they had been specified on hello command line.</span></span>

<span data-ttu-id="5798a-391">Em um arquivo de resposta, é possível especificar vários parâmetros em um único arquivo ou especificar cada parâmetro na própria linha.</span><span class="sxs-lookup"><span data-stu-id="5798a-391">In a response file, you can either specify multiple parameters on a single line, or specify each parameter on its own line.</span></span> <span data-ttu-id="5798a-392">Um parâmetro individual não pode abranger várias linhas.</span><span class="sxs-lookup"><span data-stu-id="5798a-392">Note that an individual parameter cannot span multiple lines.</span></span>

<span data-ttu-id="5798a-393">Arquivos de resposta podem incluir linhas de comentários que começam com hello # símbolo.</span><span class="sxs-lookup"><span data-stu-id="5798a-393">Response files can include comments lines that begin with hello # symbol.</span></span>

<span data-ttu-id="5798a-394">É possível especificar vários arquivos de resposta.</span><span class="sxs-lookup"><span data-stu-id="5798a-394">You can specify multiple response files.</span></span> <span data-ttu-id="5798a-395">No entanto, o AzCopy não permite arquivos de resposta aninhados.</span><span class="sxs-lookup"><span data-stu-id="5798a-395">However, note that AzCopy does not support nested response files.</span></span>

<span data-ttu-id="5798a-396">**Aplicável a:** Blobs, Arquivos, Tabelas</span><span class="sxs-lookup"><span data-stu-id="5798a-396">**Applicable to:** Blobs, Files, Tables</span></span>

### <a name="y"></a><span data-ttu-id="5798a-397">/Y</span><span class="sxs-lookup"><span data-stu-id="5798a-397">/Y</span></span>
<span data-ttu-id="5798a-398">Suprime todas as solicitações de confirmação do AzCopy.</span><span class="sxs-lookup"><span data-stu-id="5798a-398">Suppresses all AzCopy confirmation prompts.</span></span>

<span data-ttu-id="5798a-399">**Aplicável a:** Blobs, Arquivos, Tabelas</span><span class="sxs-lookup"><span data-stu-id="5798a-399">**Applicable to:** Blobs, Files, Tables</span></span>

### <a name="l"></a><span data-ttu-id="5798a-400">/L</span><span class="sxs-lookup"><span data-stu-id="5798a-400">/L</span></span>
<span data-ttu-id="5798a-401">Especifica uma operação de listagem apenas. Nenhum dado é copiado.</span><span class="sxs-lookup"><span data-stu-id="5798a-401">Specifies a listing operation only; no data is copied.</span></span>

<span data-ttu-id="5798a-402">AzCopy interpreta Olá usar essa opção como uma simulação de linha de comando Olá em execução sem essa opção /L e as contagens de quantos objetos são copiados, você pode especificar opção /V no Olá a mesma hora toocheck quais objetos são copiados no log detalhado hello.</span><span class="sxs-lookup"><span data-stu-id="5798a-402">AzCopy interprets hello using of this option as a simulation for running hello command line without this option /L and counts how many objects are copied, you can specify option /V at hello same time toocheck which objects are copied in hello verbose log.</span></span>

<span data-ttu-id="5798a-403">comportamento de saudação dessa opção também é determinado pelo local Olá Olá dos dados de origem e a presença de Olá Olá recursiva modo opção /S e arquivo padrão da opção de /Pattern.</span><span class="sxs-lookup"><span data-stu-id="5798a-403">hello behavior of this option is also determined by hello location of hello source data and hello presence of hello recursive mode option /S and file pattern option /Pattern.</span></span>

<span data-ttu-id="5798a-404">O AzCopy exige a permissão de LISTAGEM e de LEITURA deste local de origem ao usar essa opção.</span><span class="sxs-lookup"><span data-stu-id="5798a-404">AzCopy requires LIST and READ permission of this source location when using this option.</span></span>

<span data-ttu-id="5798a-405">**Aplicável a:** Blobs, Arquivos</span><span class="sxs-lookup"><span data-stu-id="5798a-405">**Applicable to:** Blobs, Files</span></span>

### <a name="mt"></a><span data-ttu-id="5798a-406">/MT</span><span class="sxs-lookup"><span data-stu-id="5798a-406">/MT</span></span>
<span data-ttu-id="5798a-407">Define a hora da última modificação do arquivo baixado Olá toobe Olá mesmo como o blob de origem hello ou do arquivo.</span><span class="sxs-lookup"><span data-stu-id="5798a-407">Sets hello downloaded file's last-modified time toobe hello same as hello source blob or file's.</span></span>

<span data-ttu-id="5798a-408">**Aplicável a:** Blobs, Arquivos</span><span class="sxs-lookup"><span data-stu-id="5798a-408">**Applicable to:** Blobs, Files</span></span>

### <a name="xn"></a><span data-ttu-id="5798a-409">/XN</span><span class="sxs-lookup"><span data-stu-id="5798a-409">/XN</span></span>
<span data-ttu-id="5798a-410">Exclui um recurso de origem mais novo.</span><span class="sxs-lookup"><span data-stu-id="5798a-410">Excludes a newer source resource.</span></span> <span data-ttu-id="5798a-411">Olá recursos não são copiados se hora da última modificação do código-fonte Olá Olá é hello igual ou mais recente do que o destino.</span><span class="sxs-lookup"><span data-stu-id="5798a-411">hello resource is not copied if hello last modified time of hello source is hello same or newer than destination.</span></span>

<span data-ttu-id="5798a-412">**Aplicável a:** Blobs, Arquivos</span><span class="sxs-lookup"><span data-stu-id="5798a-412">**Applicable to:** Blobs, Files</span></span>

### <a name="xo"></a><span data-ttu-id="5798a-413">/XO</span><span class="sxs-lookup"><span data-stu-id="5798a-413">/XO</span></span>
<span data-ttu-id="5798a-414">Exclui um recurso de origem mais antigo.</span><span class="sxs-lookup"><span data-stu-id="5798a-414">Excludes an older source resource.</span></span> <span data-ttu-id="5798a-415">Olá recursos não são copiados se hora da última modificação do código-fonte Olá Olá é hello mesmo ou mais antigo que o destino.</span><span class="sxs-lookup"><span data-stu-id="5798a-415">hello resource is not copied if hello last modified time of hello source is hello same or older than destination.</span></span>

<span data-ttu-id="5798a-416">**Aplicável a:** Blobs, Arquivos</span><span class="sxs-lookup"><span data-stu-id="5798a-416">**Applicable to:** Blobs, Files</span></span>

### <a name="a"></a><span data-ttu-id="5798a-417">/A</span><span class="sxs-lookup"><span data-stu-id="5798a-417">/A</span></span>
<span data-ttu-id="5798a-418">Carrega somente os arquivos que têm o atributo de arquivo morto de saudação configurado.</span><span class="sxs-lookup"><span data-stu-id="5798a-418">Uploads only files that have hello Archive attribute set.</span></span>

<span data-ttu-id="5798a-419">**Aplicável a:** Blobs, Arquivos</span><span class="sxs-lookup"><span data-stu-id="5798a-419">**Applicable to:** Blobs, Files</span></span>

### <a name="iarashcnetoi"></a><span data-ttu-id="5798a-420">/IA:[RASHCNETOI]</span><span class="sxs-lookup"><span data-stu-id="5798a-420">/IA:[RASHCNETOI]</span></span>
<span data-ttu-id="5798a-421">Carrega somente os arquivos que têm algum Olá especificado de atributos.</span><span class="sxs-lookup"><span data-stu-id="5798a-421">Uploads only files that have any of hello specified attributes set.</span></span>

<span data-ttu-id="5798a-422">Entre os atributos disponíveis estão:</span><span class="sxs-lookup"><span data-stu-id="5798a-422">Available attributes include:</span></span>

* <span data-ttu-id="5798a-423">R = Arquivos somente leitura</span><span class="sxs-lookup"><span data-stu-id="5798a-423">R = Read-only files</span></span>
* <span data-ttu-id="5798a-424">A = Arquivos prontos para arquivamento</span><span class="sxs-lookup"><span data-stu-id="5798a-424">A = Files ready for archiving</span></span>
* <span data-ttu-id="5798a-425">S = Arquivos de sistema</span><span class="sxs-lookup"><span data-stu-id="5798a-425">S = System files</span></span>
* <span data-ttu-id="5798a-426">H = Arquivos ocultos</span><span class="sxs-lookup"><span data-stu-id="5798a-426">H = Hidden files</span></span>
* <span data-ttu-id="5798a-427">C = Arquivo compactado</span><span class="sxs-lookup"><span data-stu-id="5798a-427">C = Compressed files</span></span>
* <span data-ttu-id="5798a-428">N = Arquivos normais</span><span class="sxs-lookup"><span data-stu-id="5798a-428">N = Normal files</span></span>
* <span data-ttu-id="5798a-429">E = Arquivos criptografados</span><span class="sxs-lookup"><span data-stu-id="5798a-429">E = Encrypted files</span></span>
* <span data-ttu-id="5798a-430">T = Arquivos temporários</span><span class="sxs-lookup"><span data-stu-id="5798a-430">T = Temporary files</span></span>
* <span data-ttu-id="5798a-431">O = Arquivos offline</span><span class="sxs-lookup"><span data-stu-id="5798a-431">O = Offline files</span></span>
* <span data-ttu-id="5798a-432">I = Arquivos não indexados</span><span class="sxs-lookup"><span data-stu-id="5798a-432">I = Non-indexed files</span></span>

<span data-ttu-id="5798a-433">**Aplicável a:** Blobs, Arquivos</span><span class="sxs-lookup"><span data-stu-id="5798a-433">**Applicable to:** Blobs, Files</span></span>

### <a name="xarashcnetoi"></a><span data-ttu-id="5798a-434">/XA:[RASHCNETOI]</span><span class="sxs-lookup"><span data-stu-id="5798a-434">/XA:[RASHCNETOI]</span></span>
<span data-ttu-id="5798a-435">Exclui os arquivos que têm algum Olá especificado conjunto de atributos.</span><span class="sxs-lookup"><span data-stu-id="5798a-435">Excludes files that have any of hello specified attributes set.</span></span>

<span data-ttu-id="5798a-436">Entre os atributos disponíveis estão:</span><span class="sxs-lookup"><span data-stu-id="5798a-436">Available attributes include:</span></span>

* <span data-ttu-id="5798a-437">R = Arquivos somente leitura</span><span class="sxs-lookup"><span data-stu-id="5798a-437">R = Read-only files</span></span>
* <span data-ttu-id="5798a-438">A = Arquivos prontos para arquivamento</span><span class="sxs-lookup"><span data-stu-id="5798a-438">A = Files ready for archiving</span></span>
* <span data-ttu-id="5798a-439">S = Arquivos de sistema</span><span class="sxs-lookup"><span data-stu-id="5798a-439">S = System files</span></span>
* <span data-ttu-id="5798a-440">H = Arquivos ocultos</span><span class="sxs-lookup"><span data-stu-id="5798a-440">H = Hidden files</span></span>
* <span data-ttu-id="5798a-441">C = Arquivo compactado</span><span class="sxs-lookup"><span data-stu-id="5798a-441">C = Compressed files</span></span>
* <span data-ttu-id="5798a-442">N = Arquivos normais</span><span class="sxs-lookup"><span data-stu-id="5798a-442">N = Normal files</span></span>
* <span data-ttu-id="5798a-443">E = Arquivos criptografados</span><span class="sxs-lookup"><span data-stu-id="5798a-443">E = Encrypted files</span></span>
* <span data-ttu-id="5798a-444">T = Arquivos temporários</span><span class="sxs-lookup"><span data-stu-id="5798a-444">T = Temporary files</span></span>
* <span data-ttu-id="5798a-445">O = Arquivos offline</span><span class="sxs-lookup"><span data-stu-id="5798a-445">O = Offline files</span></span>
* <span data-ttu-id="5798a-446">I = Arquivos não indexados</span><span class="sxs-lookup"><span data-stu-id="5798a-446">I = Non-indexed files</span></span>

<span data-ttu-id="5798a-447">**Aplicável a:** Blobs, Arquivos</span><span class="sxs-lookup"><span data-stu-id="5798a-447">**Applicable to:** Blobs, Files</span></span>

### <a name="delimiterdelimiter"></a><span data-ttu-id="5798a-448">/Delimiter: "delimitador"</span><span class="sxs-lookup"><span data-stu-id="5798a-448">/Delimiter:"delimiter"</span></span>
<span data-ttu-id="5798a-449">Indica o caractere delimitador de saudação usado diretórios virtuais toodelimit em um nome de blob.</span><span class="sxs-lookup"><span data-stu-id="5798a-449">Indicates hello delimiter character used toodelimit virtual directories in a blob name.</span></span>

<span data-ttu-id="5798a-450">Por padrão, usa AzCopy / como o caractere delimitador de saudação.</span><span class="sxs-lookup"><span data-stu-id="5798a-450">By default, AzCopy uses / as hello delimiter character.</span></span> <span data-ttu-id="5798a-451">No entanto, o AzCopy dá suporte ao uso de qualquer caractere comum (como @, # ou %) como delimitador.</span><span class="sxs-lookup"><span data-stu-id="5798a-451">However, AzCopy supports using any common character (such as @, #, or %) as a delimiter.</span></span> <span data-ttu-id="5798a-452">Se você precisar tooinclude um desses caracteres especiais na linha de comando hello, inclua o nome do arquivo de saudação com aspas duplas.</span><span class="sxs-lookup"><span data-stu-id="5798a-452">If you need tooinclude one of these special characters on hello command line, enclose hello file name with double quotes.</span></span>

<span data-ttu-id="5798a-453">Essa opção só é aplicável para o download de blobs.</span><span class="sxs-lookup"><span data-stu-id="5798a-453">This option is only applicable for downloading blobs.</span></span>

<span data-ttu-id="5798a-454">**Aplicável a:** Blobs</span><span class="sxs-lookup"><span data-stu-id="5798a-454">**Applicable to:** Blobs</span></span>

### <a name="ncnumber-of-concurrent-operations"></a><span data-ttu-id="5798a-455">/NC: "número-de-operações-simultâneas"</span><span class="sxs-lookup"><span data-stu-id="5798a-455">/NC:"number-of-concurrent-operations"</span></span>
<span data-ttu-id="5798a-456">Especifica o número de saudação de operações simultâneas.</span><span class="sxs-lookup"><span data-stu-id="5798a-456">Specifies hello number of concurrent operations.</span></span>

<span data-ttu-id="5798a-457">AzCopy por padrão inicia um determinado número de operações simultâneas tooincrease Olá transferência de transferência de dados.</span><span class="sxs-lookup"><span data-stu-id="5798a-457">AzCopy by default starts a certain number of concurrent operations tooincrease hello data transfer throughput.</span></span> <span data-ttu-id="5798a-458">Observe que o grande número de operações simultâneas em um ambiente de baixa largura de banda pode sobrecarregar a conexão de rede de saudação e evitar operações de saudação de ser totalmente concluída.</span><span class="sxs-lookup"><span data-stu-id="5798a-458">Note that large number of concurrent operations in a low-bandwidth environment may overwhelm hello network connection and prevent hello operations from fully completing.</span></span> <span data-ttu-id="5798a-459">Limite operações simultâneas com base na largura de banda real de rede disponível.</span><span class="sxs-lookup"><span data-stu-id="5798a-459">Throttle concurrent operations based on actual available network bandwidth.</span></span>

<span data-ttu-id="5798a-460">limite superior de saudação para as operações simultâneas é 512.</span><span class="sxs-lookup"><span data-stu-id="5798a-460">hello upper limit for concurrent operations is 512.</span></span>

<span data-ttu-id="5798a-461">**Aplicável a:** Blobs, Arquivos, Tabelas</span><span class="sxs-lookup"><span data-stu-id="5798a-461">**Applicable to:** Blobs, Files, Tables</span></span>

### <a name="sourcetypeblob--table"></a><span data-ttu-id="5798a-462">/SourceType:"Blob" | "Table"</span><span class="sxs-lookup"><span data-stu-id="5798a-462">/SourceType:"Blob" | "Table"</span></span>
<span data-ttu-id="5798a-463">Especifica que Olá `source` recurso é um blob disponível no ambiente de desenvolvimento local hello, em execução no emulador de armazenamento hello.</span><span class="sxs-lookup"><span data-stu-id="5798a-463">Specifies that hello `source` resource is a blob available in hello local development environment, running in hello storage emulator.</span></span>

<span data-ttu-id="5798a-464">**Aplicável a:** Blobs, Tabelas</span><span class="sxs-lookup"><span data-stu-id="5798a-464">**Applicable to:** Blobs, Tables</span></span>

### <a name="desttypeblob--table"></a><span data-ttu-id="5798a-465">/DestType:"Blob" | "Table"</span><span class="sxs-lookup"><span data-stu-id="5798a-465">/DestType:"Blob" | "Table"</span></span>
<span data-ttu-id="5798a-466">Especifica que Olá `destination` recurso é um blob disponível no ambiente de desenvolvimento local hello, em execução no emulador de armazenamento hello.</span><span class="sxs-lookup"><span data-stu-id="5798a-466">Specifies that hello `destination` resource is a blob available in hello local development environment, running in hello storage emulator.</span></span>

<span data-ttu-id="5798a-467">**Aplicável a:** Blobs, Tabelas</span><span class="sxs-lookup"><span data-stu-id="5798a-467">**Applicable to:** Blobs, Tables</span></span>

### <a name="pkrskey1key2key3"></a><span data-ttu-id="5798a-468">/PKRS: "chave1#chave2#chave3#..."</span><span class="sxs-lookup"><span data-stu-id="5798a-468">/PKRS:"key1#key2#key3#..."</span></span>
<span data-ttu-id="5798a-469">Divisões Olá tooenable de intervalo de chave de partição, exportação de dados de tabela em paralelo, o que aumenta a velocidade de Olá Olá da operação de exportação.</span><span class="sxs-lookup"><span data-stu-id="5798a-469">Splits hello partition key range tooenable exporting table data in parallel, which increases hello speed of hello export operation.</span></span>

<span data-ttu-id="5798a-470">Se essa opção não for especificada, o AzCopy usa entidades de tabela tooexport de thread único.</span><span class="sxs-lookup"><span data-stu-id="5798a-470">If this option is not specified, then AzCopy uses a single thread tooexport table entities.</span></span> <span data-ttu-id="5798a-471">Por exemplo, se hello usuário Especifica /PKRS: "aa #bb" e, em seguida, AzCopy inicia três operações simultâneas.</span><span class="sxs-lookup"><span data-stu-id="5798a-471">For example, if hello user specifies /PKRS:"aa#bb", then AzCopy starts three concurrent operations.</span></span>

<span data-ttu-id="5798a-472">Cada operação exporta um dos três intervalos de chaves de partição, como mostramos abaixo:</span><span class="sxs-lookup"><span data-stu-id="5798a-472">Each operation exports one of three partition key ranges, as shown below:</span></span>

  <span data-ttu-id="5798a-473">[first-partition-key, aa)</span><span class="sxs-lookup"><span data-stu-id="5798a-473">[first-partition-key, aa)</span></span>

  <span data-ttu-id="5798a-474">[aa, bb)</span><span class="sxs-lookup"><span data-stu-id="5798a-474">[aa, bb)</span></span>

  <span data-ttu-id="5798a-475">[bb, last-partition-key]</span><span class="sxs-lookup"><span data-stu-id="5798a-475">[bb, last-partition-key]</span></span>

<span data-ttu-id="5798a-476">**Aplicável a:** Tabelas</span><span class="sxs-lookup"><span data-stu-id="5798a-476">**Applicable to:** Tables</span></span>

### <a name="splitsizefile-size"></a><span data-ttu-id="5798a-477">/Splitsize: "tamanho do arquivo"</span><span class="sxs-lookup"><span data-stu-id="5798a-477">/SplitSize:"file-size"</span></span>
<span data-ttu-id="5798a-478">Especifica Olá arquivo exportado de dividir o tamanho em MB, Olá valor mínimo permitido é 32.</span><span class="sxs-lookup"><span data-stu-id="5798a-478">Specifies hello exported file split size in MB, hello minimal value allowed is 32.</span></span>

<span data-ttu-id="5798a-479">Se essa opção não for especificada, AzCopy exporta tooa de dados da tabela de arquivo único.</span><span class="sxs-lookup"><span data-stu-id="5798a-479">If this option is not specified, AzCopy exports table data tooa single file.</span></span>

<span data-ttu-id="5798a-480">Se dados de tabela de saudação são exportado tooa blob e hello, tamanho do arquivo exportado atinge Olá limite de 200 GB para o tamanho do blob, AzCopy divide arquivo exportado do hello, mesmo se essa opção não for especificada.</span><span class="sxs-lookup"><span data-stu-id="5798a-480">If hello table data is exported tooa blob, and hello exported file size reaches hello 200 GB limit for blob size, then AzCopy splits hello exported file, even if this option is not specified.</span></span>

<span data-ttu-id="5798a-481">**Aplicável a:** Tabelas</span><span class="sxs-lookup"><span data-stu-id="5798a-481">**Applicable to:** Tables</span></span>

### <a name="entityoperationinsertorskip--insertormerge--insertorreplace"></a><span data-ttu-id="5798a-482">/EntityOperation:"InsertOrSkip" | "InsertOrMerge" | "InsertOrReplace"</span><span class="sxs-lookup"><span data-stu-id="5798a-482">/EntityOperation:"InsertOrSkip" | "InsertOrMerge" | "InsertOrReplace"</span></span>
<span data-ttu-id="5798a-483">Especifica o comportamento de importação de dados de tabela hello.</span><span class="sxs-lookup"><span data-stu-id="5798a-483">Specifies hello table data import behavior.</span></span>

* <span data-ttu-id="5798a-484">InsertOrSkip - ignora uma entidade existente ou insere uma nova entidade se ele não existe na tabela de saudação.</span><span class="sxs-lookup"><span data-stu-id="5798a-484">InsertOrSkip - Skips an existing entity or inserts a new entity if it does not exist in hello table.</span></span>
* <span data-ttu-id="5798a-485">InsertOrMerge - mescla uma entidade existente ou insere uma nova entidade se ele não existe na tabela de saudação.</span><span class="sxs-lookup"><span data-stu-id="5798a-485">InsertOrMerge - Merges an existing entity or inserts a new entity if it does not exist in hello table.</span></span>
* <span data-ttu-id="5798a-486">InsertOrReplace - substitui uma entidade existente ou insere uma nova entidade se ele não existe na tabela de saudação.</span><span class="sxs-lookup"><span data-stu-id="5798a-486">InsertOrReplace - Replaces an existing entity or inserts a new entity if it does not exist in hello table.</span></span>

<span data-ttu-id="5798a-487">**Aplicável a:** Tabelas</span><span class="sxs-lookup"><span data-stu-id="5798a-487">**Applicable to:** Tables</span></span>

### <a name="manifestmanifest-file"></a><span data-ttu-id="5798a-488">/Manifesto: "arquivo de manifesto"</span><span class="sxs-lookup"><span data-stu-id="5798a-488">/Manifest:"manifest-file"</span></span>
<span data-ttu-id="5798a-489">Especifica o arquivo de manifesto Olá para tabela Olá exportar e importar a operação.</span><span class="sxs-lookup"><span data-stu-id="5798a-489">Specifies hello manifest file for hello table export and import operation.</span></span>

<span data-ttu-id="5798a-490">Essa opção é opcional durante a operação de exportação hello, AzCopy gera um arquivo de manifesto com nome pré-definido se essa opção não for especificada.</span><span class="sxs-lookup"><span data-stu-id="5798a-490">This option is optional during hello export operation, AzCopy generates a manifest file with predefined name if this option is not specified.</span></span>

<span data-ttu-id="5798a-491">Essa opção é necessária durante a operação de importação Olá para localizar os arquivos de dados de saudação.</span><span class="sxs-lookup"><span data-stu-id="5798a-491">This option is required during hello import operation for locating hello data files.</span></span>

<span data-ttu-id="5798a-492">**Aplicável a:** Tabelas</span><span class="sxs-lookup"><span data-stu-id="5798a-492">**Applicable to:** Tables</span></span>

### <a name="synccopy"></a><span data-ttu-id="5798a-493">/SyncCopy</span><span class="sxs-lookup"><span data-stu-id="5798a-493">/SyncCopy</span></span>
<span data-ttu-id="5798a-494">Indica se toosynchronously copiar blobs ou arquivos entre dois pontos de extremidade de armazenamento do Azure.</span><span class="sxs-lookup"><span data-stu-id="5798a-494">Indicates whether toosynchronously copy blobs or files between two Azure Storage endpoints.</span></span>

<span data-ttu-id="5798a-495">O AzCopy por padrão usa cópia assíncrona no servidor.</span><span class="sxs-lookup"><span data-stu-id="5798a-495">AzCopy by default uses server-side asynchronous copy.</span></span> <span data-ttu-id="5798a-496">Especifique esse tooperform opção um síncrono copiar, que faz o download de blobs ou arquivos toolocal memória e, em seguida, carrega tooAzure armazenamento.</span><span class="sxs-lookup"><span data-stu-id="5798a-496">Specify this option tooperform a synchronous copy, which downloads blobs or files toolocal memory and then uploads them tooAzure Storage.</span></span>

<span data-ttu-id="5798a-497">Você pode usar essa opção ao copiar arquivos no armazenamento de Blob no armazenamento de arquivo ou Blob storage tooFile armazenamento ou vice-versa.</span><span class="sxs-lookup"><span data-stu-id="5798a-497">You can use this option when copying files within Blob storage, within File storage, or from Blob storage tooFile storage or vice versa.</span></span>

<span data-ttu-id="5798a-498">**Aplicável a:** Blobs, Arquivos</span><span class="sxs-lookup"><span data-stu-id="5798a-498">**Applicable to:** Blobs, Files</span></span>

### <a name="setcontenttypecontent-type"></a><span data-ttu-id="5798a-499">/SetContentType:"content-type"</span><span class="sxs-lookup"><span data-stu-id="5798a-499">/SetContentType:"content-type"</span></span>
<span data-ttu-id="5798a-500">Especifica o tipo de conteúdo MIME Olá para blobs de destino ou arquivos.</span><span class="sxs-lookup"><span data-stu-id="5798a-500">Specifies hello MIME content type for destination blobs or files.</span></span>

<span data-ttu-id="5798a-501">Conjuntos de AzCopy Olá o tipo de conteúdo para um blob ou arquivo tooapplication/octet-stream por padrão.</span><span class="sxs-lookup"><span data-stu-id="5798a-501">AzCopy sets hello content type for a blob or file tooapplication/octet-stream by default.</span></span> <span data-ttu-id="5798a-502">Você pode definir o tipo de conteúdo Olá para todos os blobs ou arquivos especificando explicitamente um valor para essa opção.</span><span class="sxs-lookup"><span data-stu-id="5798a-502">You can set hello content type for all blobs or files by explicitly specifying a value for this option.</span></span>

<span data-ttu-id="5798a-503">Se você especificar essa opção sem um valor, AzCopy define cada blob ou tipo de conteúdo do arquivo de acordo com tooits extensão de arquivo.</span><span class="sxs-lookup"><span data-stu-id="5798a-503">If you specify this option without a value, then AzCopy sets each blob or file's content type according tooits file extension.</span></span>

<span data-ttu-id="5798a-504">**Aplicável a:** Blobs, Arquivos</span><span class="sxs-lookup"><span data-stu-id="5798a-504">**Applicable to:** Blobs, Files</span></span>

### <a name="payloadformatjson--csv"></a><span data-ttu-id="5798a-505">/PayloadFormat:"JSON" | "CSV"</span><span class="sxs-lookup"><span data-stu-id="5798a-505">/PayloadFormat:"JSON" | "CSV"</span></span>
<span data-ttu-id="5798a-506">Especifica o formato de Olá Olá tabela exportada do arquivo de dados.</span><span class="sxs-lookup"><span data-stu-id="5798a-506">Specifies hello format of hello table exported data file.</span></span>

<span data-ttu-id="5798a-507">Se essa opção não for especificada, por padrão, o AzCopy exportará o arquivo de dados da tabela no formato JSON.</span><span class="sxs-lookup"><span data-stu-id="5798a-507">If this option is not specified, by default AzCopy exports table data file in JSON format.</span></span>

<span data-ttu-id="5798a-508">**Aplicável a:** Tabelas</span><span class="sxs-lookup"><span data-stu-id="5798a-508">**Applicable to:** Tables</span></span>

## <a name="known-issues-and-best-practices"></a><span data-ttu-id="5798a-509">Problemas Conhecidos e Práticas Recomendadas</span><span class="sxs-lookup"><span data-stu-id="5798a-509">Known Issues and Best Practices</span></span>
### <a name="limit-concurrent-writes-while-copying-data"></a><span data-ttu-id="5798a-510">Limite gravações simultâneas durante a cópia de dados</span><span class="sxs-lookup"><span data-stu-id="5798a-510">Limit concurrent writes while copying data</span></span>
<span data-ttu-id="5798a-511">Quando você copia blobs ou arquivos com AzCopy, tenha em mente que outro aplicativo esteja modificando dados saudação enquanto você estiver copiando-lo.</span><span class="sxs-lookup"><span data-stu-id="5798a-511">When you copy blobs or files with AzCopy, keep in mind that another application may be modifying hello data while you are copying it.</span></span> <span data-ttu-id="5798a-512">Se possível, certifique-se de que dados Olá que você está copiando não está sendo modificados durante a operação de cópia de saudação.</span><span class="sxs-lookup"><span data-stu-id="5798a-512">If possible, ensure that hello data you are copying is not being modified during hello copy operation.</span></span> <span data-ttu-id="5798a-513">Por exemplo, ao copiar um VHD associado a uma máquina virtual do Azure, certifique-se de que nenhum outro aplicativo no momento está escrevendo toohello VHD.</span><span class="sxs-lookup"><span data-stu-id="5798a-513">For example, when copying a VHD associated with an Azure virtual machine, make sure that no other applications are currently writing toohello VHD.</span></span> <span data-ttu-id="5798a-514">Toodo uma boa maneira trata concedendo Olá recurso toobe copiado.</span><span class="sxs-lookup"><span data-stu-id="5798a-514">A good way toodo this is by leasing hello resource toobe copied.</span></span> <span data-ttu-id="5798a-515">Como alternativa, você pode criar um instantâneo de saudação VHD primeiro e copie instantâneo hello.</span><span class="sxs-lookup"><span data-stu-id="5798a-515">Alternately, you can create a snapshot of hello VHD first and then copy hello snapshot.</span></span>

<span data-ttu-id="5798a-516">Se você não pode impedir que outros aplicativos gravando tooblobs ou arquivos enquanto eles estão sendo copiados, em seguida, tenha em mente que, pelo trabalho de Olá Olá tempo termina, hello recursos copiados não podem mais ter paridade completa com os recursos de origem hello.</span><span class="sxs-lookup"><span data-stu-id="5798a-516">If you cannot prevent other applications from writing tooblobs or files while they are being copied, then keep in mind that by hello time hello job finishes, hello copied resources may no longer have full parity with hello source resources.</span></span>

### <a name="run-one-azcopy-instance-on-one-machine"></a><span data-ttu-id="5798a-517">Execute uma instância de AzCopy em um computador.</span><span class="sxs-lookup"><span data-stu-id="5798a-517">Run one AzCopy instance on one machine.</span></span>
<span data-ttu-id="5798a-518">AzCopy é projetado toomaximize Olá utilização sua máquina recurso tooaccelerate Olá de transferência de dados, é recomendável executar apenas uma instância de AzCopy em um computador e especificar a opção Olá `/NC` se você precisar de operações simultâneas mais.</span><span class="sxs-lookup"><span data-stu-id="5798a-518">AzCopy is designed toomaximize hello utilization of your machine resource tooaccelerate hello data transfer, we recommend you run only one AzCopy instance on one machine, and specify hello option `/NC` if you need more concurrent operations.</span></span> <span data-ttu-id="5798a-519">Para obter mais detalhes, digite `AzCopy /?:NC` na linha de comando hello.</span><span class="sxs-lookup"><span data-stu-id="5798a-519">For more details, type `AzCopy /?:NC` at hello command line.</span></span>

### <a name="enable-fips-compliant-md5-algorithms-for-azcopy-when-you-use-fips-compliant-algorithms-for-encryption-hashing-and-signing"></a><span data-ttu-id="5798a-520">Habilite algoritmos MD5 compatíveis com FIPS para o AzCopy quando você "Usar algoritmos compatíveis com FIPS para criptografia, hash e assinatura".</span><span class="sxs-lookup"><span data-stu-id="5798a-520">Enable FIPS compliant MD5 algorithms for AzCopy when you "Use FIPS compliant algorithms for encryption, hashing and signing".</span></span>
<span data-ttu-id="5798a-521">AzCopy por padrão usa Olá toocalculate da implementação .NET MD5 MD5 ao copiar objetos, mas há alguns requisitos de segurança que precisam de configuração FIPS MD5 compatível tooenable AzCopy.</span><span class="sxs-lookup"><span data-stu-id="5798a-521">AzCopy by default uses .NET MD5 implementation toocalculate hello MD5 when copying objects, but there are some security requirements that need AzCopy tooenable FIPS compliant MD5 setting.</span></span>

<span data-ttu-id="5798a-522">Você pode criar um arquivo app.config `AzCopy.exe.config` com a propriedade `AzureStorageUseV1MD5` e reservá-lo com o AzCopy.exe.</span><span class="sxs-lookup"><span data-stu-id="5798a-522">You can create an app.config file `AzCopy.exe.config` with property `AzureStorageUseV1MD5` and put it aside with AzCopy.exe.</span></span>
...
<span data-ttu-id="5798a-523">Para a propriedade "AzureStorageUseV1MD5" • True - o valor de padrão de saudação, AzCopy usa implementação .NET MD5.</span><span class="sxs-lookup"><span data-stu-id="5798a-523">For property "AzureStorageUseV1MD5" • True - hello default value, AzCopy uses .NET MD5 implementation.</span></span>
<span data-ttu-id="5798a-524">• False – o AzCopy usa o algoritmo MD5 compatível com FIPS.</span><span class="sxs-lookup"><span data-stu-id="5798a-524">• False – AzCopy uses FIPS compliant MD5 algorithm.</span></span>

<span data-ttu-id="5798a-525">Observe que os algoritmos compatíveis com FIPS estão desabilitados por padrão em seu computador com Windows. Digite secpol.msc na janela Executar e marque essa opção em Configuração de Segurança -> Segurança -> Políticas Locais > Opções de Segurança > Criptografia do Sistema: usar algoritmos compatíveis com FIPS para criptografia, hash e assinatura.</span><span class="sxs-lookup"><span data-stu-id="5798a-525">Note that FIPS compliant algorithms is disabled by default on your Windows machine, you can type secpol.msc in your Run window and check this switch at Security Setting->Local Policy->Security Options->System cryptography: Use FIPS compliant algorithms for encryption, hashing and signing.</span></span>

## <a name="next-steps"></a><span data-ttu-id="5798a-526">Próximas etapas</span><span class="sxs-lookup"><span data-stu-id="5798a-526">Next steps</span></span>
<span data-ttu-id="5798a-527">Para obter mais informações sobre o armazenamento do Azure e AzCopy, consulte Olá recursos a seguir:</span><span class="sxs-lookup"><span data-stu-id="5798a-527">For more information about Azure Storage and AzCopy, see hello following resources:</span></span>

### <a name="azure-storage-documentation"></a><span data-ttu-id="5798a-528">Documentação do Armazenamento do Azure:</span><span class="sxs-lookup"><span data-stu-id="5798a-528">Azure Storage documentation:</span></span>
* [<span data-ttu-id="5798a-529">Introdução tooAzure armazenamento</span><span class="sxs-lookup"><span data-stu-id="5798a-529">Introduction tooAzure Storage</span></span>](../storage-introduction.md)
* [<span data-ttu-id="5798a-530">Como toouse armazenamento de BLOBs no .NET</span><span class="sxs-lookup"><span data-stu-id="5798a-530">How toouse Blob storage from .NET</span></span>](../blobs/storage-dotnet-how-to-use-blobs.md)
* [<span data-ttu-id="5798a-531">Como toouse armazenamento de arquivo do .NET</span><span class="sxs-lookup"><span data-stu-id="5798a-531">How toouse File storage from .NET</span></span>](../storage-dotnet-how-to-use-files.md)
* [<span data-ttu-id="5798a-532">Como toouse o armazenamento de tabela do .NET</span><span class="sxs-lookup"><span data-stu-id="5798a-532">How toouse Table storage from .NET</span></span>](../../cosmos-db/table-storage-how-to-use-dotnet.md)
* [<span data-ttu-id="5798a-533">Como toocreate, gerenciar ou excluir uma conta de armazenamento</span><span class="sxs-lookup"><span data-stu-id="5798a-533">How toocreate, manage, or delete a storage account</span></span>](../storage-create-storage-account.md)
* [<span data-ttu-id="5798a-534">Transferir dados com o AzCopy no Linux</span><span class="sxs-lookup"><span data-stu-id="5798a-534">Transfer data with AzCopy on Linux</span></span>](storage-use-azcopy-linux.md)

### <a name="azure-storage-blog-posts"></a><span data-ttu-id="5798a-535">Postagens de blog de armazenamento do Azure:</span><span class="sxs-lookup"><span data-stu-id="5798a-535">Azure Storage blog posts:</span></span>
* [<span data-ttu-id="5798a-536">Apresentando a versão de visualização da biblioteca de movimentação de dados do armazenamento do Azure</span><span class="sxs-lookup"><span data-stu-id="5798a-536">Introducing Azure Storage Data Movement Library Preview</span></span>](https://azure.microsoft.com/blog/introducing-azure-storage-data-movement-library-preview-2/)
* [<span data-ttu-id="5798a-537">AzCopy: Introducing synchronous copy and customized content type (AzCopy: introdução a cópia síncrona e tipo de conteúdo personalizado)</span><span class="sxs-lookup"><span data-stu-id="5798a-537">AzCopy: Introducing synchronous copy and customized content type</span></span>](http://blogs.msdn.com/b/windowsazurestorage/archive/2015/01/13/azcopy-introducing-synchronous-copy-and-customized-content-type.aspx)
* [<span data-ttu-id="5798a-538">AzCopy: Announcing General Availability of AzCopy 3.0 plus preview release of AzCopy 4.0 with Table and File support (AzCopy: anúncio de disponibilidade geral do AzCopy 3.0 mais versão de teste do AzCopy 4.0 com suporte para Tabela e Arquivo)</span><span class="sxs-lookup"><span data-stu-id="5798a-538">AzCopy: Announcing General Availability of AzCopy 3.0 plus preview release of AzCopy 4.0 with Table and File support</span></span>](http://blogs.msdn.com/b/windowsazurestorage/archive/2014/10/29/azcopy-announcing-general-availability-of-azcopy-3-0-plus-preview-release-of-azcopy-4-0-with-table-and-file-support.aspx)
* [<span data-ttu-id="5798a-539">AzCopy: Optimized for Large-Scale Copy Scenarios (AzCopy: otimizado para cenários de cópia em larga escala)</span><span class="sxs-lookup"><span data-stu-id="5798a-539">AzCopy: Optimized for Large-Scale Copy Scenarios</span></span>](http://go.microsoft.com/fwlink/?LinkId=507682)
* [<span data-ttu-id="5798a-540">AzCopy: Support for read-access geo-redundant storage (AzCopy: suporte para o armazenamento com redundância geográfica com acesso de leitura)</span><span class="sxs-lookup"><span data-stu-id="5798a-540">AzCopy: Support for read-access geo-redundant storage</span></span>](http://blogs.msdn.com/b/windowsazurestorage/archive/2014/04/07/azcopy-support-for-read-access-geo-redundant-account.aspx)
* [<span data-ttu-id="5798a-541">AzCopy: Transfer data with re-startable mode and SAS token (AzCopy: transferir dados com o modo reiniciável e token de SAS)</span><span class="sxs-lookup"><span data-stu-id="5798a-541">AzCopy: Transfer data with re-startable mode and SAS token</span></span>](http://blogs.msdn.com/b/windowsazurestorage/archive/2013/09/07/azcopy-transfer-data-with-re-startable-mode-and-sas-token.aspx)
* [<span data-ttu-id="5798a-542">AzCopy: Using cross-account Copy Blob (AzCopy: usando blob de cópia em várias contas)</span><span class="sxs-lookup"><span data-stu-id="5798a-542">AzCopy: Using cross-account Copy Blob</span></span>](http://blogs.msdn.com/b/windowsazurestorage/archive/2013/04/01/azcopy-using-cross-account-copy-blob.aspx)
* [<span data-ttu-id="5798a-543">AzCopy: Uploading/downloading files for Azure Blobs (AzCopy: Upload/download de arquivos para Blobs do Azure)</span><span class="sxs-lookup"><span data-stu-id="5798a-543">AzCopy: Uploading/downloading files for Azure Blobs</span></span>](http://blogs.msdn.com/b/windowsazurestorage/archive/2012/12/03/azcopy-uploading-downloading-files-for-windows-azure-blobs.aspx)
