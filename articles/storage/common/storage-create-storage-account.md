...
5. Selecione o tipo de saudação da conta de armazenamento: **geral** ou **armazenamento de Blob**. **Finalidade geral** é saudação padrão.
    ...
    Se **armazenamento de Blob** foi selecionado, especifique o nível de acesso de saudação: **acesso** ou **moderado**. saudação padrão é **acesso**. Confira [Armazenamento de Blobs do Azure: camadas Estática e Dinâmica](../blobs/storage-blob-storage-tiers.md) para saber mais.
...
Blob contas de armazenamento, alteração da camada de acesso de saudação podem incorrer em encargos de saudação alteram Além disso toochanging o preço. Consulte Olá [Blob contas de armazenamento - preços e faturamento](../blobs/storage-blob-storage-tiers.md#pricing-and-billing) para obter mais detalhes.
...
Se você tentar toodelete uma conta de armazenamento associada a uma máquina virtual do Azure, você pode receber um erro sobre a conta de armazenamento Olá ainda estão em uso. Para obter ajuda com a solução desse erro, confira [Solucionar erros ao excluir contas de armazenamento](../common/storage-resource-manager-cannot-delete-storage-account-container-vhd.md).
