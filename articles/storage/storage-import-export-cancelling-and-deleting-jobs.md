...
Você pode solicitar que um trabalho seja cancelado antes de ser em Olá `Packaging` estado pela chamada hello [Update Job Properties](/rest/api/storageimportexport/jobs#Jobs_Update) Olá operação e configuração `CancelRequested` elemento muito`true`. Olá trabalho será cancelado em uma base de melhor esforço. Se as unidades estiverem no processo de saudação de transferência de dados, dados podem continuar toobe transferido mesmo depois que o cancelamento foi solicitado.

 Um trabalho cancelado moverá toohello `Completed` de estado e mantidos por 90 dias, no ponto em que ela será excluída.

 toodelete um trabalho, chamada hello [excluir trabalho](/rest/api/storageimportexport/jobs#Jobs_Delete) operação antes que o trabalho Olá tiver sido enviado (*ou seja,*, enquanto o trabalho de saudação em hello `Creating` estado). Você também pode excluir um trabalho quando ele estiver em Olá `Completed` estado. Após a exclusão de um trabalho, suas informações e status não são mais acessíveis por meio da API REST de saudação ou Olá portal do Azure.
