...
Este artigo fornece sintaxe e opções para comandos de interface de linha de comando (CLI) do Azure que faria normalmente para usam toocreate e gerenciar recursos do Azure no modelo de implantação do Azure Resource Manager hello. Você pode acessar esses comandos executando Olá CLI no modo do Gerenciador de recursos (arm). Essa não é uma referência completa, e sua versão da CLI poderá mostrar comandos ou parâmetros um pouco diferentes. Para obter uma visão geral dos recursos e dos grupos de recursos do Azure, confira [Visão geral do Azure Resource Manager](../azure-resource-manager/resource-group-overview.md).  

> [!NOTE]
> Este artigo mostra Gerenciador de recursos de comandos de modo Olá CLI do Azure, às vezes chamado 1.0 da CLI do Azure. toowork no modelo do Gerenciador de recursos de saudação, você também pode tentar Olá [2.0 do CLI do Azure](/cli/azure/install-az-cli2), nossa próxima multiplataforma de geração CLI.
>Saiba mais sobre Olá [antigo e novo CLIs do Azure](/cli/azure/old-and-new-clis).
>

tooget iniciada, primeiro [instalar Olá CLI do Azure](../cli-install-nodejs.md) e [conectar tooyour assinatura do Azure](../xplat-cli-connect.md).

Para sintaxe de comando atual e as opções na linha de comando Olá no modo do Gerenciador de recursos, digite `azure help` ou toodisplay ajuda para um comando específico, `azure help [command]`. Também encontre exemplos CLI na documentação de saudação para criar e gerenciar os serviços do Azure específicos.

Parâmetros opcionais são mostrados entre colchetes (por exemplo, `[parameter]`). Todos os outros parâmetros são obrigatórios.

Parâmetros opcionais são específicas de toocommand adição documentados aqui, há três parâmetros opcionais que podem ser usado toodisplay detalhadas de saída, como opções de solicitação e códigos de status. Olá `-v` parâmetro fornece a saída detalhada e hello `-vv` parâmetro fornece ainda mais detalhado de saída detalhada. Olá `--json` opção produz resultados Olá no formato json bruto.

## <a name="setting-hello-resource-manager-mode"></a>Configuração do modo de saudação Gerenciador de recursos
Use Olá tooenable comandos de CLI do Azure Resource Manager comandos de modo a seguir.

    azure config mode arm

> [!NOTE]
> modos de Gerenciador de recursos do Azure e gerenciamento de serviços do Azure do Hello CLI são mutuamente exclusivos. Ou seja, os recursos criados no modo não podem ser gerenciados de saudação outro modo.
> 
> 

## <a name="azure-account-manage-your-account-information"></a>conta do Azure: gerenciar as informações da sua conta
As informações de assinatura do Azure são usadas pela conta do hello ferramenta tooconnect tooyour.

**Listar assinaturas Olá importado**
...
**Definir a assinatura atual Olá**

    account set [options] <subscriptionNameOrId>

**Remover uma assinatura ou o ambiente ou limpar todas as informações de conta e ambiente Olá armazenado**  

    account clear [options]

**Comandos toomanage seu ambiente de conta**  
...
## <a name="azure-ad-commands-toodisplay-active-directory-objects"></a>o ad do Azure: comandos toodisplay objetos do Active Directory
**Comandos toodisplay do active directory aplicativos**
...
**Comandos tooprovide uma informação de grupo ou um membro da sub do active directory**

    ad group member list [options] [objectId]

**Entidades de serviço comandos toodisplay do active directory**
...
**Usuários do active directory de toodisplay de comandos**

    ad user list [options]
    ad user show [options]

## <a name="azure-availset-commands-toomanage-your-availability-sets"></a>Azure availset: toomanage comandos sua disponibilidade define
**Cria um conjunto de disponibilidade dentro de um grupo de recursos**

    availset create [options] <resource-group> <name> <location> [tags]

**Listas Olá conjuntos de disponibilidade dentro de um grupo de recursos**
...
## <a name="azure-config-commands-toomanage-your-local-settings"></a>configuração do Azure: comandos toomanage as configurações locais
...
**Conjuntos Olá CLI do Azure trabalhar de modo tooeither `arm` ou`asm`**

    config mode [options] <modename>


## <a name="azure-feature-commands-toomanage-account-features"></a>o recurso do Azure: recursos de conta toomanage de comandos
...
**Grupo de recursos do conjunto de marcas tooa**
...
**Lista os grupos de recursos Olá para sua assinatura**
...
**Logs de grupo de recursos de toomanage de comandos**

    group log show [options] [name]

**Comandos toomanage sua implantação em um grupo de recursos**
...
**Comandos toomanage o modelo de grupo de recursos de galeria ou local**
...
## <a name="azure-hdinsight-commands-toomanage-your-hdinsight-clusters"></a>hdinsight do Azure: comandos toomanage seus clusters HDInsight
**Comandos toocreate ou adicionar o arquivo de configuração de cluster tooa**

    hdinsight config create [options] <configFilePath> <overwrite>
    hdinsight config add-config-values [options] <configFilePath>
    hdinsight config add-script-action [options] <configFilePath>

Exemplo: Crie um arquivo de configuração que contém um toorun de ação de script ao criar um cluster.

    hdinsight config create "C:\myFiles\configFile.config"
    hdinsight config add-script-action --configFilePath "C:\myFiles\configFile.config" --nodeType HeadNode --uri <scriptActionURI> --name myScriptAction --parameters "-param value"

**Comando toocreate um cluster em um grupo de recursos**
...
**Toolist de comando todos os clusters (em um grupo de recursos específico, se fornecida)**
...
**Acesso de tooenable HTTP para um cluster de comando**

    hdinsight cluster enable-http-access [options] <clusterName> <userName> <password>

**Acesso de toodisable HTTP para um cluster de comando**

    hdinsight cluster disable-http-access [options] <clusterName>

**Comando tooenable acesso RDP para um cluster**
...
## <a name="azure-insights-commands-related-toomonitoring-insights-events-alert-rules-autoscale-settings-metrics"></a>o Azure insights: comandos relacionados toomonitoring Insights (eventos, regras de alerta, configurações de dimensionamento automático, as métricas)
...
## <a name="azure-location-commands-tooget-hello-available-locations-for-all-resource-types"></a>local do Azure: comandos locais disponíveis do tooget Olá para todos os tipos de recurso
**Lista os locais disponíveis Olá**

    location list [options]

## <a name="azure-network-commands-toomanage-network-resources"></a>rede do Azure: comandos toomanage recursos de rede
**Comandos toomanage as redes virtuais**

    network vnet create [options] <resource-group> <name> <location>
Cria uma rede virtual. Olá exemplo a seguir é criar uma rede virtual denominado newvnet para myresourcegroup do grupo de recursos na região Oeste dos EUA de saudação.
...
comando Olá lista todas as redes virtuais em um grupo de recursos.
...
comando de saudação mostra as propriedades de rede virtual de saudação em um grupo de recursos.
...
comando Olá remove uma rede virtual.
...
**Sub-redes de rede virtual toomanage comandos**

    network vnet subnet create [options] <resource-group> <vnet-name> <name>

Adiciona outro subrede tooan rede virtual existente.
...
Lista todas as sub-redes de rede virtual Olá para uma rede virtual específica dentro de um grupo de recursos.
...
**Balanceadores de carga toomanage comandos**
...
**Testes de toomanage de comandos de um balanceador de carga**

    network lb probe create [options] <resource-group> <lb-name> <name>

Crie a configuração de investigação de saudação para status de integridade no balanceador de carga de saudação. Tenha em mente toorun esse comando, o balanceador de carga requer um recurso de ip de front-end (Check-out tooassign do comando "azure rede front-end-ip" um balanceador de tooload de endereço ip).
...
Propriedades de investigação de saudação da lista para um conjunto de Balanceador de carga.
...
Remove a investigação Olá criada Olá balanceador de carga.
...
**Configurações de ip de front-end de toomanage de comandos de um balanceador de carga**

    network lb frontend-ip create [options] <resource-group> <lb-name> <name>
Cria um tooan de configuração de IP de front-end existente conjunto do balanceador de carga.
...
Atualizações de uma configuração existente de um front-end IP.hello comando a seguir adiciona um IP público chamado mypubip5 tooan existente carga balanceador IP front-end denominado myfrontendip.
...
Lista todos os recursos IP de front-end de saudação configurados Olá balanceador de carga.
...
Exclui Olá front-end IP objeto associado tooload balanceador
...
**Comandos toomanage pools de endereços de back-end de um balanceador de carga**
...
<BR>
    excluir o pool de endereços de balanceamento de carga de rede [Opções] < resource-group >< lb-name ><name>

Remove o recurso de intervalo de pool IP do hello back-end do balanceador de carga.
...
**Comandos toomanage regras de Balanceador de carga**

    network lb rule create [options] <resource-group> <lb-name> <name>
Crie regras para o balanceador de carga.

Você pode criar uma regra de Balanceador de carga Configurando o ponto de extremidade do hello front-end do balanceador de carga de saudação e Olá back-end endereço pool intervalo tooreceive Olá tráfego de rede. As configurações também incluem portas Olá para o ponto de extremidade IP de front-end e portas para o intervalo de pool de endereços de back-end de saudação.

Olá exemplo a seguir mostra como regra toocreate um balanceador de carga, ponto de extremidade de front-end de saudação escutando tooport 80 tooport 8080 para o intervalo de pool de endereços de back-end de saudação de envio de tráfego de rede de TCP e o balanceamento de carga.
...
Atualiza uma regra de balanceador de carga existente definida em um grupo de recursos específico. Olá exemplo a seguir, alteramos o nome da regra de saudação do mylbrule toomynewlbrule.
...
**Regras NAT de entrada do balanceador de carga toomanage comandos**

    network lb inbound-nat-rule create [options] <resource-group> <lb-name> <name>
Cria uma regra de NAT de entrada para o balanceador de carga.

Em Olá o exemplo a seguir criamos uma regra NAT de IP de front-end (que foi anteriormente definida usando o comando hello "ip de front-end de rede do azure") com uma porta de escuta de entrada e a porta de saída que balanceador de carga Olá usa tráfego de rede toosend hello.
...
Atualiza uma regra de nat de entrada existente. Em Olá exemplo a seguir, alteramos Olá porta de escuta de 80 too81 de entrada.
...
Exclui a regra NAT Olá balanceador de carga em um grupo de recursos específicos.
...
**Endereços de ip público toomanage comandos**

    network public-ip create [options] <resource-group> <name> <location>
Cria um recurso de ip público. Você criará o recurso de ip público de saudação e associar o nome de domínio tooa.
...
Atualiza as propriedades de saudação de um recurso de ip público existente. Saudação de exemplo a seguir alteramos o endereço IP público de saudação do tooStatic dinâmico.
...
Opções de parâmetro:
    ...
    -s, --subscription <subscription>      hello subscription identifier
<BR>
    rede ip público Mostrar [Opções] < grupo de recursos ><name>
...
**Interfaces de rede toomanage comandos**

    network nic create [options] <resource-group> <name> <location>
Cria um recurso chamado de interface de rede (NIC) que pode ser usada para balanceadores de carga ou associar tooa Máquina Virtual.
...
**Comandos toomanage regras de grupo de segurança de rede**
...
**Perfil do Gerenciador de tráfego de toomanage de comandos**
...
**Pontos de extremidade de comandos toomanage traffic manager**

    network traffic-manager profile endpoint create [options] <resource-group> <profile-name> <name> <endpoint-location>
    network traffic-manager profile endpoint set [options] <resource-group> <profile-name> <name>
    network traffic-manager profile endpoint delete [options] <resource-group> <profile-name> <name>

**Gateways de rede virtual toomanage comandos**

    network gateway list [options] <resource-group>

## <a name="azure-provider-commands-toomanage-resource-provider-registrations"></a>o provedor do Azure: comandos toomanage registros de provedor de recursos
**Liste os provedores registrados atualmente no Resource Manager**

    provider list [options]

**Mostrar detalhes sobre Olá solicitado namespace do provedor**

    provider show [options] <namespace>

**Registre o provedor com assinatura de saudação**

    provider register [options] <namespace>

**Cancelar o registro do provedor com assinatura Olá**

    provider unregister [options] <namespace>

## <a name="azure-resource-commands-toomanage-your-resources"></a>recursos do Azure: comandos toomanage seus recursos
...
**Comandos toomanage sua atribuição de função**

    role assignment create [options] [objectId] [upn] [mail] [spn] [role] [scope] [resource-group] [resource-type] [resource-name]
    role assignment list [options] [objectId] [upn] [mail] [spn] [role] [scope] [resource-group] [resource-type] [resource-name]
    role assignment delete [options] [objectId] [upn] [mail] [spn] [role] [scope] [resource-group] [resource-type] [resource-name]

## <a name="azure-storage-commands-toomanage-your-storage-objects"></a>armazenamento do Azure: comandos toomanage seus objetos de armazenamento
**Comandos toomanage suas contas de armazenamento**
...
**Comandos toomanage suas chaves de conta de armazenamento**

    storage account keys list [options] <name>
    storage account keys renew [options] <name>

**Comandos tooshow sua cadeia de caracteres de conexão de armazenamento**

    storage account connectionstring show [options] <name>

**Comandos toomanage seus contêineres de armazenamento**
...
**Assinaturas de acesso compartilhado de toomanage comandos do seu contêiner de armazenamento**

    storage container sas create [options] [container] [permissions] [expiry]

**Comandos toomanage armazenado políticas de acesso do seu contêiner de armazenamento**
...
**Comandos toomanage seus blobs de armazenamento**
...
**Comandos toomanage suas operações de cópia de blob**

    storage blob copy start [options] [sourceUri] [destContainer]
    storage blob copy show [options] [container] [blob]
    storage blob copy stop [options] [container] [blob] [copyid]

**Assinatura de acesso compartilhado toomanage comandos do seu blob de armazenamento**

    storage blob sas create [options] [container] [blob] [permissions] [expiry]

**Comandos toomanage seus compartilhamentos de arquivos de armazenamento**
...
**Comandos toomanage seus arquivos de armazenamento**
...
**Comandos toomanage seu diretório do arquivo de armazenamento**

    storage directory create [options] [share] [path]
    storage directory delete [options] [share] [path]

**Comandos toomanage suas filas de armazenamento**
...
**Assinaturas de acesso comandos toomanage compartilhado de sua fila de armazenamento**

    storage queue sas create [options] [queue] [permissions] [expiry]

**Comandos toomanage armazenado políticas de acesso de sua fila de armazenamento**
...
**Comandos toomanage suas propriedades de log de armazenamento**

    storage logging show [options]
    storage logging set [options]

**Comandos toomanage suas propriedades de métricas de armazenamento**

    storage metrics show [options]
    storage metrics set [options]

**Comandos toomanage suas tabelas de armazenamento**
...
**Assinaturas de acesso compartilhado de toomanage comandos de sua tabela de armazenamento**

    storage table sas create [options] [table] [permissions] [expiry]

**Comandos toomanage armazenado políticas de acesso de sua tabela de armazenamento**
...
## <a name="azure-tag-commands-toomanage-your-resource-manager-tag"></a>marca do Azure: comandos toomanage sua marca de Gerenciador de recursos
**Adicione uma marca**
...
**Lista informações de marca Olá**
...
## <a name="azure-vm-commands-toomanage-your-azure-virtual-machines"></a>vm do Azure: comandos toomanage máquinas virtuais do Azure
...
> [!TIP]
> A partir do CLI versão 0,10, você pode fornecer um alias curto, como "UbuntuLTS" ou "Win2012R2Datacenter" como Olá `image-urn` para algumas imagens do Marketplace populares. Execute `azure help vm quick-create` para opções. Além disso, começando com a versão 0,10, `azure vm quick-create` usa o armazenamento premium por padrão, se ele está disponível na região selecionada da saudação.
> 
> 

**Listar máquinas virtuais de saudação em uma conta**
...
**Desligar uma máquina virtual em uma saudação de grupo e as versões do recurso de recursos de computação**
...
**Capturar Olá VM como imagem do sistema operacional ou imagem de VM**

    vm capture [options] <resource-group> <name> <vhd-name-prefix>

**Definir estado de saudação do hello VM tooGeneralized**

    vm generalize [options] <resource-group> <name>

**Obter exibição de instância de saudação VM**

    vm get-instance-view [options] <resource-group> <name>

**Habilitar o acesso de área de trabalho remota ou SSH configurações em uma máquina Virtual e tooreset Olá senha para conta Olá com o administrador ou sudo autoridade tooreset**
...
**Comandos toomanage os discos de dados de máquina Virtual**

    vm disk attach-new [options] <resource-group> <vm-name> <size-in-gb> [vhd-name]
    vm disk detach [options] <resource-group> <vm-name> <lun>
    vm disk attach [options] <resource-group> <vm-name> [vhd-url]

**Extensões de recurso VM comandos toomanage**

    vm extension set [options] <resource-group> <vm-name> <name> <publisher-name> <version>
    vm extension get [options] <resource-group> <vm-name>

**Comandos toomanage sua máquina Virtual do Docker**
