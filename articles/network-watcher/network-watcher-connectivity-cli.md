...
Verificação de conectividade está atualmente em visualização pública, toouse esse recurso que é necessário toobe registrado. toodo, Olá execução seguinte exemplo CLI
...
tooverify Olá o registro foi bem-sucedido, execute Olá CLI comando a seguir:
...
Olá json a seguir é de resposta de exemplo hello de executar o cmdlet anterior hello. Como a seleção de saudação for bem-sucedida, Olá `connectionStatus` propriedade mostra como **alcançável**.  São fornecidos detalhes de Olá relativos ao número de saudação do blob de armazenamento saltos tooreach necessário hello e latência.
