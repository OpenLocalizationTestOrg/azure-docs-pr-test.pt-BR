...
Agora que temos informações Olá necessário toorun Olá cmdlet, executamos Olá `az network watcher test-ip-flow` tráfego de saudação do cmdlet tootest. Neste exemplo, estamos usando endereço IP primeiro Olá em Olá primeira NIC.
...
Depois de executar `az network watcher test-ip-flow` Olá resultados são retornados, hello, exemplo a seguir é resultados Olá de saudação anterior da etapa.
