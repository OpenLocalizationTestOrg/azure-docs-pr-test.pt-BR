...
| TCP 9354 |Saída |WAN |Sim |porta de saída de saudação é usada pelo Olá toocommunicate de dispositivo StorSimple com hello serviço StorSimple Manager. |
...
Olá algoritmos de métrica de roteamento é diferente dependendo da versão do software Olá em execução no seu dispositivo StorSimple.

**Versões anterior tooUpdate 1**

Isso inclui a versão do software versões anteriores tooUpdate 1 como Olá GA, 0.1, 0.2 ou 0,3. ordem de saudação com base nas métricas de roteamentos é o seguinte:
...
**Atualização 1 e 2 do tooUpdate anterior a partir de versões**

Isso inclui versões de software, como 1, 1.1 e 1.2. ordem de saudação com base nas métricas de roteamentos é decidido da seguinte maneira:
   ...
   *DADOS 0 > Adaptador de rede de 10 GbE configurado por último > Outros adaptadores de rede de 10 GbE > Último adaptador de rede de 1 GbE configurado > Outro adaptador de rede de 1 GbE*
   ...
   Atualização 1, métrica de roteamento de saudação de DATA 0 é feita hello mais baixo; Portanto, tráfego de nuvem de saudação todos os é roteada por meio de dados 0. Anote isso se houver mais de uma interface de rede habilitada para nuvem em seu dispositivo StorSimple.

**Versões a partir da Atualização 2**

Atualização 2 tem vários aprimoramentos relacionado à rede e métricas de roteamento Olá foi alterado. comportamento de saudação pode ser explicado como a seguir.
...
* ordem de saudação tráfego de nuvem hello será roteado por meio das interfaces de rede de saudação é:
    ...
    *onde Olá números entre parênteses indicam métricas roteamento do respectivos hello.*
...
* Um alerta também será gerado no dispositivo do StorSimple quando houver uma falha de VIP. Para obter mais informações, vá muito[referência rápida de alerta](storsimple-manage-alerts.md).
...
* Isole o tráfego iSCSI e nuvem Olá tendo interfaces de rede em seu dispositivo para acesso iSCSI e nuvem dedicadas. Para obter mais informações, consulte como muito[modificar interfaces de rede](storsimple-modify-device-config.md#modify-network-interfaces) em seu dispositivo StorSimple.
...
Para obter mais informações sobre como monitorar e manter componentes de hardware de saudação do seu dispositivo StorSimple, ir muito[usar componentes de hardware toomonitor do serviço de Gerenciador do StorSimple hello e status](storsimple-monitor-hardware-status.md) e [StorSimple substituição de componentes de hardware](storsimple-hardware-component-replacement.md).
...
* Durante a substituição, monitore Olá **Status do Hardware** em Olá **manutenção** página Olá unidades no hello SSDs e HDDs. Um status de verificação verde indica que os discos Olá estão íntegros ou Okey, enquanto o ponto de um ponto de exclamação vermelho indica uma falha SSD ou HDD.
...
* Durante uma substituição de módulo do controlador EBOD, monitorar continuamente o status de saudação de componente Olá Olá serviço StorSimple Manager acessando **manutenção** > **status do Hardware**.
...
* [Saiba como toodeploy sua solução StorSimple](storsimple-deployment-walkthrough-u2.md).
