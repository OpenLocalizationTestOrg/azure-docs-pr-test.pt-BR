...
Bem-vindo à implantação de dispositivo Azure StorSimple tooMicrosoft. Esses tutoriais de implantação se aplicam a tooStorSimple 8000 StorSimple 8000 Series atualização 0.3, StorSimple 8000 Series atualização 0,1, StorSimple 8000 Series atualização 0,2 e versão de série. Esta série de tutoriais descreve como tooconfigure seu dispositivo StorSimple e inclui uma lista de verificação de configuração, pré-requisitos de configuração e a configuração detalhada etapas.
...
Olá seções a seguir explica os pré-requisitos de configuração Olá para rede Olá em seu data center, seu dispositivo StorSimple e seu serviço StorSimple Manager.
...
* dispositivo de saudação em seu data center pode se conectar a rede toooutside. Execute seguinte Olá [Windows PowerShell 4.0](http://www.microsoft.com/download/details.aspx?id=40855) cmdlets (tabulados abaixo) toovalidate Olá conectividade toohello fora da rede. Execute essa validação em um computador que tenha conectividade tooAzure (na rede de datacenter) e onde você implantará o dispositivo StorSimple.  

| Para este parâmetro... | validade do hello toocheck... | Execute estes comandos/cmdlets. |
| --- | --- | --- |
| **IP**</br>**Sub-rede**</br>**Gateway** |É um endereço IPv4 ou IPv6 válido?</br>Trata-se de uma sub-rede válida?</br>Trata-se de um gateway válido?</br>Este é um IP duplicado na rede? |`ping ip`</br>`arp -a`</br>Olá `ping` e `arp` comandos devem falhar, indicando que não há nenhum dispositivo de rede de datacenter Olá que está usando esse IP. |
|  | | |
...
| &nbsp; |Verifique se a porta 53 está aberta. Isso é aplicável somente se você estiver usando um DNS externo para o seu dispositivo. DNS interno deve resolver URLs externas Olá automaticamente. |`Test-Port -comp dc1 -port 53 -udp -UDPtimeout 10000`  </br>[Mais informações sobre esse cmdlet](http://learn-powershell.net/2011/02/21/querying-udp-ports-with-powershell/) |
|  | | |
...
|  | | |
| **Proxy (opcional)** |É uma porta e um URI de proxy válido? </br> Modo de autenticação hello está correto? |<code>wget http://bing.com &#124; % {$_.StatusCode}</code></br>Esse comando deve ser executado imediatamente depois de configurar o proxy web. Se um código de status 200 é retornado, indica que a conexão de saudação é bem-sucedida. |
| &nbsp; |O tráfego é roteável por meio do proxy? |Execute a validação de DNS hello, seleção NTP ou verificação de HTTP, uma vez depois de configurar o proxy em seu dispositivo. Isso lhe fornecerá uma visão clara de se o tráfego está sendo bloqueado no proxy ou em outro lugar. |
|  | | |
...
Um serviço StorSimple Manager pode gerenciar vários dispositivos StorSimple. Para implantação de saudação do primeiro dispositivo StorSimple, você precisará toocreate um novo serviço StorSimple Manager.

> [!IMPORTANT]
> Ignore esta etapa se você tiver um serviço StorSimple Manager existente e você pretende toodeploy seu dispositivo StorSimple com esse serviço.
> 
> 

Execute Olá etapas toocreate uma nova instância do serviço do StorSimple Manager Olá a seguir.
...
> [!IMPORTANT]
> Tooperforming anterior nessa configuração, desconecte todas as interfaces de rede Olá exceto a DATA 0 em ambos os controladores de saudação (ativos e passivos).
> 
> 
...
Após a conclusão da configuração do dispositivo Olá, você deve verificar se há atualizações e se estiver disponível, instale as atualizações. Olá atualizações podem demorar várias toocomplete de horas. Siga as instruções de saudação em [examinar e aplicar atualizações](#scan-for-and-apply-updates).
...
> [!IMPORTANT]
> * Para alta disponibilidade da sua solução StorSimple hello, recomendamos que você configure o MPIO em seu iSCSI de tooconfiguring anterior (opcional) de host do Windows Server no host do Windows Server. Configuração do MPIO nos servidores host garantirá que os servidores de saudação podem tolerar um link, rede ou falhas de interface.
> * Para iSCSI e MPIO instruções de instalação e configuração, vá muito[configurar MPIO para seu dispositivo StorSimple](storsimple-configure-mpio-windows-server.md). Eles também serão incluem hello etapas toomount, inicializar e formatar volumes do StorSimple.
> 
> 

Se você decidir não tooconfigure MPIO, executar Olá toomount as etapas a seguir, inicializar e formatar seus volumes do StorSimple.
...
Atualizar seu dispositivo pode levar de 1 a 4 horas. Executar Olá tooscan etapas para a seguir e aplique as atualizações em seu dispositivo.

> [!NOTE]
> Se você tiver um gateway configurado em uma interface de rede diferente de Data 0, você precisará toodisable interfaces de rede Data 2 e Data 3 antes de instalar a atualização de saudação. Vá muito**dispositivos > configurar** e desabilitar as interfaces Data 2 e 3. Você deve reabilitar essas interfaces depois Olá dispositivo está atualizado.
> 
> 
...
2. É criado um tooscan de trabalho atualizações disponíveis. Se houver atualizações disponíveis, Olá **verificar atualizações** muda muito**instalar atualizações de**. Clique em **Instalar Atualizações**. Você pode ser solicitado toodisable Data 2 e Data 3 tooinstalling anterior Olá atualizações. Você deve desabilitar essas interfaces de rede ou Olá atualizações podem falhar.
3. Será criado um trabalho de atualização. Monitorar o status de saudação de sua atualização navegando muito**trabalhos**.
   
   > [!NOTE]
   > Quando o trabalho de atualização de saudação é iniciado, ele imediatamente exibe status Olá como 50 por cento. status de saudação, em seguida, altera o percentual de too100 somente depois Olá atualização trabalho seja concluído. Não há nenhum status em tempo real para o processo de atualizações de saudação.
   > 
   > 
...
* Saudação de uso [o serviço StorSimple Manager](https://msdn.microsoft.com/library/azure/dn772396.aspx) toomanage seu dispositivo StorSimple.
