...
Este tutorial explica como tooconfigure CHAP para seu dispositivo StorSimple. procedimento de saudação detalhado neste artigo se aplica a 8000 série tooStorSimple, bem como dispositivos de StorSimple 1200.

CHAP significa Challenge Handshake Authentication Protocol. É um esquema de autenticação usado por servidores toovalidate Olá identidade de clientes remotos. verificação de saudação baseia-se em uma senha secreta ou compartilhada. O protocolo CHAP pode ser unidirecional ou bidirecional (mútuo). O CHAP é unidirecional quando Olá destino autentica um iniciador. CHAP mútuo ou reverso, no hello o outro lado, requer que o destino Olá autentique o iniciador de saudação e, em seguida, o iniciador Olá autentique o destino de saudação. A autenticação do iniciador pode ser implementada sem a autenticação do destino. No entanto, a autenticação do destino pode ser implementada somente se a autenticação do iniciador também for implementada. 
...
1. Em Olá portal clássico do Azure, em Olá **dispositivos** , clique em Olá **configurar** guia.
   
    ![Iniciador CHAP](./media/storsimple-configure-chap/IC740943.png)
2. Role para baixo nesta página e em Olá **iniciador CHAP** seção:
   
   1. Forneça um nome de usuário para o iniciador CHAP.
   2. Forneça uma senha para o iniciador CHAP.
      
    > [!IMPORTANT]
    > nome de usuário CHAP Olá deve conter menos de 233 caracteres. senha CHAP de saudação deve ter entre 12 e 16 caracteres. Um nome de usuário ou a senha mais longo resultará em uma falha de autenticação no host do Windows hello.
   
   3. Confirme senha hello.
3. Clique em **Salvar**. Uma mensagem de confirmação será exibida. Clique em **Okey** toosave alterações de saudação.
...
5. Em Olá **destinos** guia da saudação **propriedades do iniciador iSCSI** janela, o status do dispositivo Olá devem aparecer como **conectado**. Se você estiver usando um dispositivo StorSimple 1200, cada volume será montado como um destino iSCSI conforme mostrado abaixo. Portanto, as etapas 3 a 4 precisará toobe repetido para cada volume.
   
    ![Volumes montados como destinos separados](./media/storsimple-configure-chap/chap4.png)
   
   > [!IMPORTANT]
   > Se você alterar o nome do iSCSI Olá, nome novo da saudação será usado para novas sessões do iSCSI. Novas configurações não são usadas para sessões existentes até que você faça logoff e logon novamente.
   > 
   > 
...
Na autenticação bidirecional, destino Olá autentica o iniciador hello e, em seguida, o iniciador Olá autentica o destino de saudação. Isso requer configurações do iniciador do CHAP tooconfigure Olá Olá usuário, bem como Olá reverter as configurações de CHAP no dispositivo de saudação e de software iSCSI no host de saudação. Olá procedimentos a seguir descrevem autenticação mútua do hello etapas tooconfigure no dispositivo de saudação e no host do Windows hello.
...
3. Em Olá **iniciador CHAP** seção:
   ...
   3. Confirme senha hello.
4. Clique em **Salvar**. Uma mensagem de confirmação será exibida. Clique em **Okey** toosave alterações de saudação.
...
4. Em Olá **segredo CHAP mútuo do iniciador de iSCSI** caixa de diálogo:
   ...
   1. Saudação de tipo **senha do CHAP reverso** que você configurou no hello portal clássico do Azure.
   2. Clique em **OK**.
...
Em Olá **conectar tooTarget** caixa de diálogo que é apresentado, selecione hello **adicionar esta lista de toohello de conexão de destinos favoritos** caixa de seleção. Isso garante que sempre Olá computador for reiniciado, uma tentativa de destinos favoritos do iSCSI de toohello de conexão do toorestore hello.
