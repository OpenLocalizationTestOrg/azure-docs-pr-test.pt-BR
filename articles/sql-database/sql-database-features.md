...
Banco de dados do SQL Azure compartilha uma base de código comum com o SQL Server e, no nível de banco de dados hello, oferece suporte à maioria de saudação mesmos recursos. Olá principais diferenças entre o banco de dados do SQL Azure e SQL Server estão no nível de instância de saudação. 

Continuamos tooadd recursos tooAzure banco de dados SQL. Para nós o incentivamos você toovisit nosso serviço atualizações página da Web para o Azure e toouse seus filtros:

* Filtrado toohello [serviço de banco de dados SQL](https://azure.microsoft.com/updates/?service=sql-database).
* Filtrado tooGeneral disponibilidade [comunicados (GA)](http://azure.microsoft.com/updates/?service=sql-database&update-type=general-availability) para recursos de banco de dados SQL.

## <a name="sql-server-and-sql-database-feature-support"></a>Suporte ao recurso Banco de Dados SQL e SQL Server

Olá tabela a seguir lista os recursos principais de saudação do SQL Server e fornece informações sobre se há suporte para cada recurso específico e um link toomore sobre o recurso de saudação. Para tooconsider de diferenças de Transact-SQL ao migrar uma solução de banco de dados existente, consulte [diferenças de Transact-SQL Resolvendo durante a migração tooSQL banco de dados](sql-database-transact-sql-information.md).
...
- Para obter informações sobre suporte ao Transact-SQL e as diferenças, consulte [diferenças de Transact-SQL Resolvendo durante a migração tooSQL banco de dados](sql-database-transact-sql-information.md).
