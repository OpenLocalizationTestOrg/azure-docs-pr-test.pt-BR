...
* Área do assembly - circuito do saudação padrão é que um Pi se conecta com um sensor BME280 e um LED. área de saudação é bloqueada na versão de visualização assim no momento você não pode fazer a personalização.
* Codificação área - um editor de código online para você toocode com framboesa Pi. aplicativo de exemplo Hello padrão ajuda a toocollect os dados do sensor do sensor BME280 e envia tooyour Azure IoT Hub. aplicativo Hello é totalmente compatível com dispositivos de Pi reais. 
* Janela de console integrado - ele mostra a saída de saudação do seu código. Na parte superior do hello desta janela, há três botões.
